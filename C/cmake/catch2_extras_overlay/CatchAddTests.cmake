# SAO overlay of Catch2 v3.5.4 extras/CatchAddTests.cmake.
# Local patch: re-group flattened PROPERTIES key/value tokens so multi-value
# LABELS survive discovery (see the `set(properties ...)` block below).
...
function(catch_discover_tests_impl)
  ...
  # SAO overlay patch (Catch2 v3.5.4 base): upstream flattens multi-value
  # PROPERTIES into bare tokens, so `LABELS "rt_io;characterization"` is
  # generated as `LABELS rt_io characterization` and every label after the
  # first is silently dropped. Re-group flattened key/value pairs: property
  # keys are UPPER_SNAKE tokens (repo labels are lowercase), and consecutive
  # value tokens are folded back into one semicolon-escaped element so
  # add_command emits them as a single argument. Values popped from the list
  # have their escapes consumed, so re-escape before folding the next token.
  set(properties)
  set(_sao_in_value OFF)
  foreach(_sao_item IN LISTS _TEST_PROPERTIES)
    if(_sao_item MATCHES "^[A-Z][A-Z0-9_]*$")
      list(APPEND properties "${_sao_item}")
      set(_sao_in_value OFF)
    elseif(_sao_in_value)
      list(POP_BACK properties _sao_prev_value)
      string(REPLACE ";" "\\;" _sao_prev_value "${_sao_prev_value}")
      list(APPEND properties "${_sao_prev_value}\;${_sao_item}")
    else()
      list(APPEND properties "${_sao_item}")
      set(_sao_in_value ON)
    endif()
  endforeach()

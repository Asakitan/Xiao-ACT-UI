...
sao_plugins_binding_angel_register_sdk(asIScriptEngine* engine) {
    ...
    return sao_plugins_binding_dispatch_provider(
        language_host_kind::angel,
        language_binding_operation::runtime_register,
        &request);
}
...
sao_plugins_binding_angel_bind_ctx(asIScriptEngine* engine,
    ...
    if (engine == nullptr || ctx == nullptr) return SAO_ERR_INVALID_ARGUMENT;
    language_binding_request request{};
    request.runtime = engine;
    request.context = reinterpret_cast<void*>(ctx);
    return sao_plugins_binding_dispatch_provider(
        language_host_kind::angel, language_binding_operation::context_bind,
        &request);
}
...
sao_plugins_binding_angel_json_to_dict(asIScriptEngine* engine,
    ...
    if (engine == nullptr || utf8_json == nullptr) return nullptr;
    language_binding_request request{};
    request.runtime = engine;
...
sao_plugins_binding_angel_activate(plugin_context_ptr plugin_ctx,
    ...
    *out_plugin = nullptr;
    return sao_plugins_binding_plugin_load(
        language_host_kind::angel, reinterpret_cast<void*>(plugin_ctx), engine,
        out_plugin);
}

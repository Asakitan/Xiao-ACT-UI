...
struct CallbackState final {
    sao_plugins_log_callback_t legacy_callback{};
    sao_plugins_structured_log_callback_t structured_callback{};
    void* user_data{};
...
[[nodiscard]] int32_t set_callback(sao_plugins_log_callback_t legacy_callback,
                                   sao_plugins_structured_log_callback_t structured_callback,
...
extern "C" int32_t SAO_PLUGINS_CALL sao_plugins_set_structured_log_callback(
    ...
    if (status != SAO_OK) {
        sao::legacy_plugins::emit_log(sao::legacy_plugins::kLogLevelError, "plugins.logging",
                                      status, "set_structured_log_callback failed");
    }
    return status;
}

extern "C" void SAO_PLUGINS_CALL sao_plugins_set_log_callback(sao_plugins_log_callback_t callback) {
    const int32_t status = set_callback(callback, nullptr, nullptr);
    if (status != SAO_OK) {
        sao::legacy_plugins::emit_log(sao::legacy_plugins::kLogLevelError, "plugins.logging",
                                      status, "set_log_callback failed");
    }
}

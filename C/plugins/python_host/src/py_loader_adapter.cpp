...
public:
    ...
    ~gil_guard() { PyGILState_Release(state_); }
...
public:
    ...
    PyGILState_STATE state() const { return state_; }
...
std::string distribution_name(std::string line) {
    ...
    const auto stop = std::min(
        extra == std::string::npos ? line.size() : extra,
...
std::string import_name_for(const std::string& distribution) {
    ...
    if (lower == "pillow") return "PIL";
    if (lower == "pyyaml") return "yaml";
    if (lower == "beautifulsoup4") return "bs4";
    if (lower == "opencv-python-headless" || lower == "opencv-python") {
        return "cv2";
    }
    if (lower == "skia-python") return "skia";
    if (lower == "protobuf") return "google.protobuf";
    return distribution;
}

bool extension_module_present(const fs::path& root,
    ...
    while (start < import_name.size()) {
        ...
        relative /= fs::u8path(import_name.substr(
    ...
    if (!fs::is_directory(parent, error)) return false;
...
int32_t requirements_report(const wchar_t* plugin_dir,
                            std::string& output) {
    output.clear();
    ...
    try {
        ...
        for (const auto* name : {L"engine", L"libs", L"vendor"}) {
            ...
            const fs::path reported = error ? candidate.lexically_normal()
                                            : canonical;
            error.clear();
            const char* label = name == std::wstring(L"engine")
                                    ? "engine"
                                    : (name == std::wstring(L"libs")
                                           ? "libs"
                                           : "vendor");
        ...
        if (fs::is_regular_file(requirements, error)) {
            ...
            while (std::getline(input, line)) {
                ...
                const auto found = std::find_if(
                    ...
                    [&import_name](const search_root& item) {
                        return extension_module_present(item.path, import_name);
                    });
                report["deps"][distribution] =
                    found == roots.end() ? "missing" : found->label;
            }
        }
        output = report.dump();
        return SAO_OK;
...
public:
    ...
    py_plugin_handle_t python_plugin() const { return python_plugin_; }

    void assign(py_loader_adapter_owner_s* owner,
    ...
    void reset() {
        ...
        if (found != owner_->plugins.end() &&
            found->second.python_plugin == python_plugin_ &&
...
int32_t acquire_plugin(void* host_user_data,
    ...
    if (found == owner->plugins.end() ||
        found->second.python_plugin == nullptr) {
...
void retain_last_error(py_loader_adapter_owner_s* owner,
                       loader_plugin_handle_t plugin,
                       std::string error) {
    std::lock_guard lock(g_adapter_mutex);
...
void finish_failed_load(py_loader_adapter_owner_s* owner,
                        loader_plugin_handle_t plugin,
                        std::string error) {
    std::lock_guard lock(g_adapter_mutex);
    ...
    try {
        ...
        if (!utf8_to_wide(manifest->source_path, plugin_dir)) {
            finish_failed_load(owner, plugin,
                               "plugin source_path is not valid UTF-8");
            return SAO_ERR_INVALID_ARGUMENT;
        }
        std::string report;
        ...
        {
            gil_guard gil;
            // host_user_data is the adapter owner.  The loader creates its
            // plugin_context_t only after load_plugin returns, while pyhost's
            // ctx_ptr ABI accepts SaoSdkContext*.  A host-owned SDK context is
            // therefore the only valid context at this point.
        ...
        if (status != SAO_OK) {
            finish_failed_load(owner, plugin,
                               error.empty() ? "Python plugin load failed"
                                             : std::move(error));
            return status;
        }
        {
...
int32_t SAO_PLUGINS_CALL adapter_on_load(loader_plugin_handle_t plugin,
    ...
    try {
        return with_plugin(
            plugin, host_user_data, [](py_plugin_handle_t python_plugin) {
...
int32_t SAO_PLUGINS_CALL adapter_on_unload(loader_plugin_handle_t plugin,
    ...
    try {
        return with_plugin(
            ...
            [allow_unload](py_plugin_handle_t python_plugin) -> int32_t {
                if (!sao_plugins_pyhost_has_hook(python_plugin, "on_unload")) {
                    return SAO_OK;
                }
                char* result = nullptr;
                const int32_t status = sao_plugins_pyhost_call_hook(
                    python_plugin, "on_unload", nullptr, &result);
                if (status == SAO_OK && result != nullptr &&
                    allow_unload != nullptr) {
...
int32_t SAO_PLUGINS_CALL adapter_unload(loader_plugin_handle_t plugin,
    ...
    try {
        ...
        std::string error;
        {
            ...
            if (status != SAO_OK) error = copy_plugin_error(python_plugin);
        }
        std::lock_guard lock(g_adapter_mutex);
        const auto found = owner->plugins.find(plugin);
        if (status != SAO_OK) {
            owner->last_errors[plugin] = error.empty()
                                             ? "Python plugin unload failed"
                                             : error;
            if (found != owner->plugins.end()) {
                found->second.lifecycle = adapter_plugin_record::state::ready;
            }
...
sao::plugins::loader::host_adapter_vtable adapter_vtable(
    py_loader_adapter_owner_s* owner) {
...
sao_plugins_pyhost_unregister_loader_adapter(py_loader_adapter_owner_t owner) {
    ...
    try {
        ...
        owner->active = false;
        lock.unlock();
        const int32_t status = sao_plugins_pyhost_shutdown(owner->host);
        if (status != SAO_OK) {
#if defined(SAO_HAS_PYTHON_EMBED)
            owner->released_thread_state = PyEval_SaveThread();
#endif
...
sao_plugins_pyhost_loader_adapter_get_requirements_report(
    ...
    void* loader_plugin_handle,
    char** out_json_utf8) {
    if (out_json_utf8 == nullptr) return SAO_ERR_INVALID_ARGUMENT;

...
std::string request(sao_ai_editor_launcher_t launcher, const std::string& payload,
                    uint32_t timeout_ms = 1000) {
    std::array<char, 256> response{};
    uint32_t length = 0;
    REQUIRE(sao_ai_editor_request(launcher, payload.data(), static_cast<uint32_t>(payload.size()),
                                  response.data(), static_cast<uint32_t>(response.size()), &length,
                                  timeout_ms) == SAO_AI_EDITOR_OK);
    return std::string(response.data(), length);
}

} // namespace

TEST_CASE("ai editor launches real configured child and handshakes", "[ai_editor][process]") {
    ...
    REQUIRE(sao_ai_editor_status(launcher.value, &running, &exit_code) == SAO_AI_EDITOR_OK);
    REQUIRE(running);
    REQUIRE(sao_ai_editor_launch(launcher.value, nullptr) == SAO_AI_EDITOR_ERR_ALREADY_RUNNING);
}
...
TEST_CASE("ai editor real child request response preserves caller buffer ABI",
    ...
    uint32_t required = 0;
    REQUIRE(sao_ai_editor_request(launcher.value, second.data(),
                                  static_cast<uint32_t>(second.size()), tiny.data(),
                                  static_cast<uint32_t>(tiny.size()), &required,
                                  1000) == SAO_AI_EDITOR_ERR_BUFFER_TOO_SMALL);
    REQUIRE(required == std::string("response:" + second).size());

    std::array<char, 128> response{};
    uint32_t length = 0;
    REQUIRE(sao_ai_editor_request(launcher.value, nullptr, 0, response.data(),
                                  static_cast<uint32_t>(response.size()), &length,
                                  1000) == SAO_AI_EDITOR_OK);
    REQUIRE(std::string(response.data(), length) == "response:" + second);
}

TEST_CASE("ai editor request timeout is bounded", "[ai_editor][process][timeout]") {
    FixtureConfig strings;
    const auto config = strings.make();
    LauncherGuard launcher;
    REQUIRE(sao_ai_editor_create(&config, &launcher.value) == SAO_AI_EDITOR_OK);
    ...
    REQUIRE(sao_ai_editor_request(launcher.value, "timeout", 7, response.data(),
                                  static_cast<uint32_t>(response.size()), &length,
                                  100) == SAO_AI_EDITOR_ERR_IPC_TIMEOUT);
    REQUIRE(GetTickCount64() - started < 2000);
}

TEST_CASE("ai editor detects child exit during request", "[ai_editor][process][exit]") {
    FixtureConfig strings;
    const auto config = strings.make();
    LauncherGuard launcher;
    REQUIRE(sao_ai_editor_create(&config, &launcher.value) == SAO_AI_EDITOR_OK);
    ...
    std::array<char, 32> response{};
    uint32_t length = 0;
    REQUIRE(sao_ai_editor_request(launcher.value, "exit-now", 8, response.data(),
                                  static_cast<uint32_t>(response.size()), &length,
                                  1000) == SAO_AI_EDITOR_ERR_IPC_CLOSED);
...
TEST_CASE("ai editor launch timeout and early exit fail closed",
    ...
    SECTION("child never connects") {
        ...
        LauncherGuard launcher;
        REQUIRE(sao_ai_editor_create(&config, &launcher.value) == SAO_AI_EDITOR_OK);
        REQUIRE(sao_ai_editor_launch(launcher.value, nullptr) == SAO_AI_EDITOR_ERR_IPC_TIMEOUT);
    }
    ...
    SECTION("child exits before connecting") {
        ...
        REQUIRE(sao_ai_editor_create(&config, &launcher.value) == SAO_AI_EDITOR_OK);
        int32_t exit_code = -1;
        REQUIRE(sao_ai_editor_launch(launcher.value, &exit_code) == SAO_AI_EDITOR_ERR_IPC_CLOSED);
...
TEST_CASE("ai editor shutdown and destroy clean up child process",
    ...
    int32_t exit_code = -1;
    REQUIRE(sao_ai_editor_shutdown(launcher.value, 2000, &exit_code) == SAO_AI_EDITOR_OK);
    REQUIRE(exit_code == 0);
...
TEST_CASE("ai editor same handle relaunches after shutdown", "[ai_editor][process][relaunch]") {
    FixtureConfig strings;
    const auto config = strings.make();
    LauncherGuard launcher;
    REQUIRE(sao_ai_editor_create(&config, &launcher.value) == SAO_AI_EDITOR_OK);
    ...
    const std::string pending = "pending-before-shutdown";
    std::array<char, 1> tiny{};
    uint32_t required = 0;
    REQUIRE(sao_ai_editor_request(launcher.value, pending.data(),
                                  static_cast<uint32_t>(pending.size()), tiny.data(),
                                  static_cast<uint32_t>(tiny.size()), &required,
                                  1000) == SAO_AI_EDITOR_ERR_BUFFER_TOO_SMALL);
    REQUIRE(required == std::string("response:" + pending).size());
    ...
    REQUIRE(sao_ai_editor_launch(launcher.value, nullptr) == SAO_AI_EDITOR_OK);
    REQUIRE(request(launcher.value, "after-shutdown") == "response:after-shutdown");
    REQUIRE(sao_ai_editor_shutdown(launcher.value, 2000, &exit_code) == SAO_AI_EDITOR_OK);
    REQUIRE(exit_code == 0);
}

TEST_CASE("ai editor same handle relaunches after abnormal exit",
          "[ai_editor][process][exit][relaunch]") {
    FixtureConfig strings;
    const auto config = strings.make();
    LauncherGuard launcher;
    REQUIRE(sao_ai_editor_create(&config, &launcher.value) == SAO_AI_EDITOR_OK);
    ...
    int32_t shutdown_exit = -1;
    REQUIRE(sao_ai_editor_shutdown(launcher.value, 100, &shutdown_exit) == SAO_AI_EDITOR_OK);
    REQUIRE(shutdown_exit == 23);

    REQUIRE(sao_ai_editor_launch(launcher.value, nullptr) == SAO_AI_EDITOR_OK);
    REQUIRE(request(launcher.value, "after-abnormal-exit") == "response:after-abnormal-exit");
    REQUIRE(sao_ai_editor_shutdown(launcher.value, 2000, &exit_code) == SAO_AI_EDITOR_OK);
    REQUIRE(exit_code == 0);
}

TEST_CASE("ai editor missing executable and base directory fail closed", "[ai_editor][config]") {
    LauncherGuard launcher;
    REQUIRE(sao_ai_editor_create(nullptr, &launcher.value) == SAO_AI_EDITOR_ERR_CONFIG_MISSING);

...
void append_error(Json& errors, std::string_view path, std::string_view reason) {
    errors.push_back(Json{{"path", std::string(path)}, {"reason", std::string(reason)}});
}
...
NumericOrder invert_numeric_order(NumericOrder order) noexcept {
    if (order == NumericOrder::less)
        return NumericOrder::greater;
    if (order == NumericOrder::greater)
        return NumericOrder::less;
    return NumericOrder::equal;
}

NumericOrder compare_unsigned_to_double(uint64_t value, double limit) noexcept {
    if (std::isnan(limit))
        return NumericOrder::equal;
    if (limit < 0.0)
        return NumericOrder::greater;
...
NumericOrder compare_signed_to_double(int64_t value, double limit) noexcept {
    if (std::isnan(limit))
        return NumericOrder::equal;
    if (value >= 0) {
    ...
    return invert_numeric_order(compare_unsigned_to_double(magnitude, -limit));
}
...
NumericOrder compare_numbers(const Json& left, const Json& right) noexcept {
    ...
    if (right.is_number_unsigned()) {
        return invert_numeric_order(compare_unsigned_to_double(right.get<uint64_t>(), value));
    }
    if (right.is_number_integer()) {
        return invert_numeric_order(compare_signed_to_double(right.get<int64_t>(), value));
    }
...
bool parse_schema_size_limit(const Json& schema, std::string_view key, const std::string& path,
                             Json& errors, uint64_t& output) {
    const Json& value = schema[std::string(key)];
    if (!value.is_number_integer()) {
        append_error(errors, path,
                     "schema " + std::string(key) + " must be a non-negative integer");
        return false;
    }
...
void validate_recursive(const Json& value, const Json& schema, const std::string& path,
                        Json& errors) {
    if (!schema.is_object()) {
    ...
    const bool valid_minimum_items =
        ...
        parse_schema_size_limit(schema, "minItems", path, errors, minimum_items);
    const bool valid_maximum_items =
        !schema.contains("maxItems") ||
        parse_schema_size_limit(schema, "maxItems", path, errors, maximum_items);
    const bool valid_minimum_length =
        !schema.contains("minLength") ||
        parse_schema_size_limit(schema, "minLength", path, errors, minimum_length);
    const bool valid_maximum_length =
        !schema.contains("maxLength") ||
        parse_schema_size_limit(schema, "maxLength", path, errors, maximum_length);
    ...
    if (schema.contains("type")) {
        ...
        if (type_field.is_string()) {
            ...
            if (!value_matches_type(value, expected)) {
                append_error(errors, path,
                             "type expected " + expected + ", got " + type_name(value));
            ...
            if (!matched) {
                append_error(errors, path, "type expected " + joined + ", got " + type_name(value));
                return;
            }
        }
    }
    ...
    if (schema.contains("enum") && schema["enum"].is_array()) {
        ...
        for (const auto& candidate : allowed) {
            const bool numeric_match =
                ...
                (!candidate.is_number_float() || std::isfinite(candidate.get<double>())) &&
    ...
    if (value.is_number() && schema.contains("minimum") && schema["minimum"].is_number()) {
        if (compare_numbers(value, schema["minimum"]) == NumericOrder::less) {
            append_error(errors, path, "number below minimum");
        }
    }
    if (value.is_number() && schema.contains("maximum") && schema["maximum"].is_number()) {
    ...
    if (schema.contains("minItems")) {
        if (valid_minimum_items && value.is_array() &&
    ...
    if (schema.contains("maxItems")) {
        if (valid_maximum_items && value.is_array() &&
    ...
    if (schema.contains("minLength")) {
        if (valid_minimum_length && value.is_string() &&
            static_cast<uint64_t>(value.get_ref<const std::string&>().size()) < minimum_length) {
            append_error(errors, path, "string shorter than minLength");
        }
    }
    if (schema.contains("maxLength")) {
        if (valid_maximum_length && value.is_string() &&
            static_cast<uint64_t>(value.get_ref<const std::string&>().size()) > maximum_length) {
    ...
    if (value.is_object() && schema.contains("required") && schema["required"].is_array()) {
        for (const auto& field : schema["required"]) {
            if (!field.is_string())
                continue;
            const std::string name = field.get<std::string>();
            if (!value.contains(name)) {
                const std::string child_path = path + (path == "$" ? "." : ".") + name;
    ...
    if (value.is_object() && schema.contains("additionalProperties") &&
        ...
        for (const auto& entry : value.items()) {
            if (!properties.is_object() || !properties.contains(entry.key())) {
                const std::string child_path = path + (path == "$" ? "." : ".") + entry.key();
    ...
    if (value.is_object() && schema.contains("properties") && schema["properties"].is_object()) {
        for (auto entry = schema["properties"].begin(); entry != schema["properties"].end();
             ++entry) {
            const std::string& field = entry.key();
            if (!value.contains(field))
                continue;
            const std::string child_path = path + (path == "$" ? "." : ".") + field;
    ...
    if (value.is_array() && schema.contains("items")) {
        ...
        if (items_schema.is_object()) {
            for (size_t index = 0; index < value.size(); ++index) {
                ...
                validate_recursive(value[index], items_schema, child_path, errors);
            }
        }
    }
}

Json tool_descriptor(std::string_view name, std::string_view description, bool read_only,
                     const Json& properties, const Json& required = Json::array()) {
        ...
        return Json{
            ...
            {"properties",
             {{"pid", {{"type", "integer"}, {"minimum", 1}}},
              {"outputDir", {{"type", "string"}, {"minLength", 1}, {"maxLength", 32767}}},
              {"maxClasses", {{"type", "integer"}, {"minimum", 1}, {"maximum", maximum_classes}}},
              {"timeoutMs", {{"type", "integer"}, {"minimum", 1}, {"maximum", 300000}}},
              {"confirmed", {{"type", "boolean"}}}}},
            {"required", Json::array({"pid", "outputDir"})}};
    }
    return Json{};
}
...
std::string relative_utf8(const std::filesystem::path& path, const std::filesystem::path& root) {
...
NativeToolRegistry::NativeToolRegistry(const ScopeStore& scopes, uint32_t maximum_file_bytes,
                                       uint32_t maximum_search_results) noexcept
    : scopes_(scopes),
      maximum_file_bytes_(maximum_file_bytes == 0 ? kDefaultMaximumFileBytes : maximum_file_bytes),
      maximum_search_results_(maximum_search_results == 0 ? kDefaultSearchResults
                                                          : maximum_search_results) {}

Json NativeToolRegistry::describe(std::string_view mode) const {
    Json tools = Json::array({
        tool_descriptor("readFile", "Read a UTF-8 workspace file", true,
                        {{"path", {{"type", "string"}}},
                         {"startLine", {{"type", "integer"}}},
                         {"endLine", {{"type", "integer"}}}},
                        {"path"}),
        tool_descriptor("listFiles", "List workspace files and directories", true,
                        {{"path", {{"type", "string"}}},
                         {"pattern", {{"type", "string"}}},
                         {"recursive", {{"type", "boolean"}}},
                         {"limit", {{"type", "integer"}}}}),
        tool_descriptor("searchFiles", "Search UTF-8 workspace files", true,
                        {{"query", {{"type", "string"}}},
                         {"path", {{"type", "string"}}},
                         {"pattern", {{"type", "string"}}},
                         {"regex", {{"type", "boolean"}}},
                         ...
                         {"limit", {{"type", "integer"}}}},
                        {"query"}),
        tool_descriptor("editFile", "Create or replace a UTF-8 workspace file", false,
                        {{"path", {{"type", "string"}}},
                         {"content", {{"type", "string"}}},
                         {"startLine", {{"type", "integer"}}},
                         {"endLine", {{"type", "integer"}}},
        ...
        tool_descriptor(
            ...
            {{"pid", {{"type", "integer"}, {"minimum", 1}}},
             {"outputDir", {{"type", "string"}, {"minLength", 1}, {"maxLength", 32767}}},
             {"maxClasses", {{"type", "integer"}, {"minimum", 1}, {"maximum", 10000}}},
             {"timeoutMs", {{"type", "integer"}, {"minimum", 1}, {"maximum", 300000}}},
             {"confirmed", {{"type", "boolean"}}}},
            {"pid", "outputDir"}),
        tool_descriptor(
            "sdkDumper.source", "Export bounded Source ClientClass netvars into the workspace",
            false,
            {{"pid", {{"type", "integer"}, {"minimum", 1}}},
             {"outputDir", {{"type", "string"}, {"minLength", 1}, {"maxLength", 32767}}},
             {"maxClasses", {{"type", "integer"}, {"minimum", 1}, {"maximum", 2000}}},
             {"timeoutMs", {{"type", "integer"}, {"minimum", 1}, {"maximum", 300000}}},
             {"confirmed", {{"type", "boolean"}}}},
            {"pid", "outputDir"}),
    });
    for (auto& tool : tools) {
        ...
        if (tool_name == "sdkDumper.unreal" || tool_name == "sdkDumper.source") {
            tool["explicitConfirmationRequired"] = true;
        }
    }
    ...
    std::vector<Json> alias_snapshot;
    {
        ...
        for (const auto& entry : aliases_) {
            ...
            Json descriptor{{"name", alias_name}, {"aliasOf", target_name}};
            if (is_builtin_name(target_name)) {
                ...
                if (target_name == "sdkDumper.unreal" || target_name == "sdkDumper.source") {
                    descriptor["explicitConfirmationRequired"] = true;
                }
            } else {
...
int32_t NativeToolRegistry::execute(std::string_view mode, std::string_view name,
                                    const Json& arguments, Json& result) const {
    ...
    bool custom_explicit_confirmation_required = false;
    ...
    if (is_builtin_name(resolved_name)) {
        ...
        if (is_builtin_name(resolved_name)) {
            ...
            if (found != custom_tools_.end()) {
                ...
                custom_explicit_confirmation_required =
                    found->second.explicit_confirmation_required;
                is_custom = true;
                found_tool = true;
            }
        }
    ...
    if (name == "readFile") {
        ...
        if (mode == "plan" && !arguments.value("confirmed", false)) {
            result = Json{{"confirmationRequired", true}, {"tool", "editFile"}};
        ...
        if (!arguments.value("confirmed", false)) {
            result = Json{
                {"confirmationRequired", true}, {"tool", std::string(name)}, {"explicit", true}};
            return SAO_AI_EDITOR_ERR_CONFIRMATION_REQUIRED;
        }
        dispatch_status = dump_sdk(arguments, result, name == "sdkDumper.unreal");
    } else if (is_gpu_hunt) {
        ...
        if (custom_explicit_confirmation_required && !arguments.value("confirmed", false)) {
        ...
        if (!custom_read_only && mode == "plan" && !arguments.value("confirmed", false)) {
            result =
                Json{{"confirmationRequired", true}, {"tool", std::string(name)}, {"custom", true}};
            return SAO_AI_EDITOR_ERR_CONFIRMATION_REQUIRED;
        }
        if (custom_execute_fn != nullptr) {
            dispatch_status = custom_execute_fn(arguments, result, custom_execute_user);
        } else {
            result = Json{{"custom", true}, {"name", std::string(name)}, {"arguments", arguments}};
    ...
    if (dispatch_status == SAO_AI_EDITOR_OK && filter_registry_ != nullptr) {
        ...
        const bool any_fired =
            filter_registry_->apply_filters(std::string(name), arguments, result, fired);
        if (any_fired && result.is_object()) {
            ...
            if (!result.contains("compressionInfo") || !result["compressionInfo"].is_object()) {
...
void NativeToolRegistry::set_filter_registry(const ToolResultFilterRegistry* registry) noexcept {
...
int32_t NativeToolRegistry::register_custom(std::string_view name, std::string_view description,
                                            ...
                                            CustomExecuteFn execute_fn, void* execute_user,
...
int32_t
NativeToolRegistry::upsert_custom_batch(const std::vector<CustomToolDescriptor>& descriptors,
    ...
    for (const auto& descriptor : descriptors) {
        if (descriptor.name.empty() || !valid_utf8(descriptor.name) ||
            is_builtin_name(descriptor.name) || is_gpu_hunt_tool_name(descriptor.name) ||
            !names.emplace(descriptor.name).second) {
            return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;
        }
        if (!descriptor.parameters.is_null() && !descriptor.parameters.is_object()) {
            return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;
        }
    }
    ...
    for (const auto& descriptor : descriptors) {
        ...
        if (existing != custom_tools_.end() && existing->second.owner != owner) {
            return SAO_AI_EDITOR_ERR_BUSY;
        }
    }
    ...
    for (const auto& descriptor : descriptors) {
        ...
        tool.explicit_confirmation_required = descriptor.explicit_confirmation_required;
        tool.owner = owner;
...
int32_t NativeToolRegistry::remove_custom_batch(const std::vector<std::string>& names,
                                                CustomToolOwner owner) {
    ...
    for (const auto& name : names) {
        if (name.empty() || !valid_utf8(name) || !unique_names.emplace(name).second) {
            return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;
        }
    }
...
int32_t NativeToolRegistry::register_alias(std::string_view alias, std::string_view target) {
    if (alias.empty() || !valid_utf8(alias) || target.empty() || !valid_utf8(target)) {
        return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;
    }
    if (alias == target) {
    ...
    // custom / other alias.
    if (is_builtin_name(alias) || is_gpu_hunt_tool_name(alias) ||
    ...
    const bool target_ok = is_builtin_name(target) || is_gpu_hunt_tool_name(target) ||
...
int32_t NativeToolRegistry::register_hook(std::string_view id, std::string_view phase,
                                          const std::vector<std::string>& tool_filter,
                                          std::string_view emit_event) {
    if (id.empty() || !valid_utf8(id) || emit_event.empty() || !valid_utf8(emit_event)) {
        return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;
    }
...
NativeToolRegistry::snapshot_hooks(std::string_view phase, std::string_view tool_name) const {
...
int32_t NativeToolRegistry::read_file(const Json& arguments, Json& result) const {
    if (!arguments.contains("path") || !arguments["path"].is_string()) {
        return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;
    }
    std::filesystem::path path;
    if (!resolve_bounded_path(scopes_.workspace_root(),
                              arguments["path"].get_ref<const std::string&>(), false, path)) {
        return SAO_AI_EDITOR_ERR_BOUNDARY_VIOLATION;
    }
    std::string content;
    const int32_t status =
        read_text_file_bounded(scopes_.workspace_root(), path, maximum_file_bytes_, content);
...
int32_t NativeToolRegistry::list_files(const Json& arguments, Json& result) const {
    ...
    const uint32_t limit =
        std::clamp(arguments.value("limit", maximum_search_results_), 1U, maximum_search_results_);
    Json entries = Json::array();
    auto append = [&](const std::filesystem::directory_entry& entry) {
        if (entries.size() >= limit || !wildcard_match(entry.path().filename().native(), pattern)) {
            return;
        }
        std::filesystem::path bounded;
        const std::string absolute = wide_to_utf8(entry.path().native());
        if (!resolve_bounded_path(scopes_.workspace_root(), absolute, false, bounded)) {
            return;
        }
...
int32_t NativeToolRegistry::search_files(const Json& arguments, Json& result) const {
    ...
    std::filesystem::path root;
    if (!resolve_bounded_path(scopes_.workspace_root(), arguments.value("path", "."), false,
                              root)) {
        return SAO_AI_EDITOR_ERR_BOUNDARY_VIOLATION;
    }
    ...
    for (const auto& entry : std::filesystem::recursive_directory_iterator(root, error)) {
        ...
        std::string content;
        if (read_text_file_bounded(scopes_.workspace_root(), bounded, maximum_file_bytes_,
...
int32_t NativeToolRegistry::edit_file(const Json& arguments, Json& result) const {
    if (!arguments.contains("path") || !arguments["path"].is_string() ||
    ...
    std::filesystem::path path;
    if (!resolve_bounded_path(scopes_.workspace_root(),
                              arguments["path"].get_ref<const std::string&>(), true, path)) {
        return SAO_AI_EDITOR_ERR_BOUNDARY_VIOLATION;
    }
    ...
    if (start > 0) {
        std::string existing;
        int32_t status =
            read_text_file_bounded(scopes_.workspace_root(), path, maximum_file_bytes_, existing);
        if (status != SAO_AI_EDITOR_OK) {
            return status;
        }
        ...
        if (start > static_cast<int64_t>(lines.size()) + 1 || (end > 0 && end < start)) {
            return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;
        }
        ...
        const auto first =
            lines.begin() + std::min<size_t>(static_cast<size_t>(start - 1), lines.size());
        ...
        lines.insert(lines.begin() + static_cast<ptrdiff_t>(start - 1), replacement.begin(),
                     replacement.end());
    ...
    if (!expected_content.has_value()) {
        ...
        if (read_status == SAO_AI_EDITOR_OK) {
            expected_content = std::move(existing);
        } else if (read_status != SAO_AI_EDITOR_ERR_NOT_FOUND) {
            return read_status;
        }
    }
    const int32_t status = expected_content.has_value()
                               ? write_text_atomic_bounded_if_unchanged(
                                     scopes_.workspace_root(), path, *expected_content, content)
                               : create_text_atomic_bounded(scopes_.workspace_root(), path,
                                                            content);
    if (status == SAO_AI_EDITOR_OK) {
        result = Json{{"ok", true},
...
int32_t NativeToolRegistry::dump_sdk(const Json& arguments, Json& result, bool unreal) const {
    if (!arguments.contains("pid") || !arguments["pid"].is_number_integer() ||
        !arguments.contains("outputDir") || !arguments["outputDir"].is_string()) {
        return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;
    }

    const auto bounded_unsigned = [](const Json& value, std::uint64_t maximum,
                                     std::uint64_t& output) {
        if (value.is_number_unsigned()) {
    ...
    std::uint64_t pid_value = 0u;
    if (!bounded_unsigned(arguments["pid"], (std::numeric_limits<std::uint32_t>::max)(),
                          pid_value) ||
        pid_value == 0u) {
        return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;
    }

    const std::string& requested_output = arguments["outputDir"].get_ref<const std::string&>();
    std::filesystem::path output_directory;
    if (!resolve_bounded_path(scopes_.workspace_root(), requested_output, true, output_directory)) {
        return SAO_AI_EDITOR_ERR_BOUNDARY_VIOLATION;
    }

    const std::uint64_t maximum_class_limit = unreal ? 10000u : 2000u;
    std::uint64_t maximum_classes = maximum_class_limit;
    std::uint64_t timeout_ms = 60000u;
    if ((arguments.contains("maxClasses") &&
         !bounded_unsigned(arguments["maxClasses"], maximum_class_limit, maximum_classes)) ||
        (arguments.contains("timeoutMs") &&
         !bounded_unsigned(arguments["timeoutMs"], 300000u, timeout_ms)) ||
        maximum_classes == 0u || timeout_ms == 0u) {
        return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;
    }

    using sao::ai_editor::sdk_dumper::DumpConfig;
    using sao::ai_editor::sdk_dumper::DumpKind;
    std::unique_ptr<sao::ai_editor::sdk_dumper::DumperBase> dumper(
        sao::ai_editor::sdk_dumper::create_dumper(unreal ? DumpKind::Unreal : DumpKind::Source));
    if (!dumper)
        return SAO_AI_EDITOR_ERR_OS_CALL_FAILED;

    DumpConfig config{};
    config.target_pid = static_cast<std::uint32_t>(pid_value);
    config.workspace_root = wide_to_utf8(scopes_.workspace_root().native());
    config.output_dir = wide_to_utf8(output_directory.native());
    config.max_classes = static_cast<std::uint32_t>(maximum_classes);
    config.timeout_ms = static_cast<std::uint32_t>(timeout_ms);
    if (config.workspace_root.empty() || config.output_dir.empty())
        return SAO_AI_EDITOR_ERR_INVALID_ARGUMENT;

    const auto dump = dumper->dump(config);
    Json files = Json::array();
    for (const std::string& output_file : dump.output_files) {
        std::filesystem::path bounded;
        if (!resolve_bounded_path(scopes_.workspace_root(), output_file, false, bounded)) {
            return SAO_AI_EDITOR_ERR_BOUNDARY_VIOLATION;
        }
        files.push_back(relative_utf8(bounded, scopes_.workspace_root()));
    }
    result = Json{{"ok", dump.ok},
                  {"engine", unreal ? "unreal" : "source"},
                  {"pid", static_cast<std::uint32_t>(pid_value)},
                  {"classCount", dump.class_count},

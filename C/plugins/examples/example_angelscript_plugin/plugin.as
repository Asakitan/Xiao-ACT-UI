...
dictionary@ render_panel(dictionary@ payload)
{
    ...
    array<dictionary@> statusItems = {
        ctx.ui.kv("语言", "AngelScript (asIScriptEngine)"),
        ctx.ui.kv("问候语", greeting),
        ctx.ui.kv("点击次数", "" + click_count),
    };
    ...
    array<dictionary@> sections = {
        ctx.ui.section("状态", statusItems),
        ctx.ui.row(buttons),
    };

    return ctx.ui.panel("AngelScript Plugin", sections);
}

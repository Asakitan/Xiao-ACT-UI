...
void scope::set(const std::string& name, emma_value value) {
    auto it = vars_.find(name);
    if (it != vars_.end()) { it->second = std::move(value); return; }
...
bool is_numeric(const emma_value& v) {
    return std::holds_alternative<int64_t>(v) || std::holds_alternative<double>(v) || std::holds_alternative<bool>(v);
}
...
void interpreter::impl::exec_node(node_id id, const std::shared_ptr<scope>& scp) {
    ...
    switch (k) {
        ...
        case node_kind::fn_def: {
            ...
            fn->body = { body_id };
            fn->closure = scp;
            // fn->interp 由 call_fn 通过 lambda 拿到 (下面的 host_impl 是可选的; 我们通过 impl 状态直接调 call_fn)
            if (!fn->name.empty()) {
        ...
        case node_kind::while_stmt: {
            ...
            while (truthy(eval_node(cond_id, scp)) && guard < 1'000'000) {
                try {
                    ...
                    if (sig.kind == signal_kind::brk) break;
                    if (sig.kind == signal_kind::cont) { ++guard; continue; }
                    throw;
                }
                ++guard;
            }
        ...
        case node_kind::for_stmt: {
            ...
            if (std::holds_alternative<std::shared_ptr<emma_list>>(iter_v)) {
                ...
                for (auto& item : arr->items) {
                    ...
                    try { exec_block(body_id, scp); }
                    catch (runtime_signal& sig) {
                        if (sig.kind == signal_kind::brk) return;
                        if (sig.kind == signal_kind::cont) continue;
                        throw;
                    }
                }
...
emma_value interpreter::impl::eval_node(node_id id, const std::shared_ptr<scope>& scp) {
    node_kind k = pool->kinds[id];
    switch (k) {
        case node_kind::literal_nil:    return nullptr;
        case node_kind::literal_bool:   return static_cast<bool>(pool->ints[id] != 0);
        case node_kind::literal_int:    return pool->ints[id];
        case node_kind::literal_float:  return pool->floats[id];
        case node_kind::literal_string: return pool->strings[id];
        ...
        case node_kind::unaryop: {
            ...
            if (op == "-") {
                if (std::holds_alternative<int64_t>(inner)) return -std::get<int64_t>(inner);
                if (std::holds_alternative<double>(inner))  return -std::get<double>(inner);
                if (std::holds_alternative<bool>(inner))    return static_cast<int64_t>(std::get<bool>(inner) ? -1 : 0);
        ...
        case node_kind::index: {
            ...
            if (std::holds_alternative<std::shared_ptr<emma_list>>(obj)) {
                ...
                if (idx < 0 || idx >= static_cast<int64_t>(arr->items.size())) return nullptr;
                return arr->items[static_cast<size_t>(idx)];
            }
            if (std::holds_alternative<std::shared_ptr<emma_dict>>(obj)) {
                auto& d = std::get<std::shared_ptr<emma_dict>>(obj);
                if (!d) return nullptr;
                std::string k = std::holds_alternative<std::string>(key)
            ...
            if (std::holds_alternative<std::string>(obj)) {
                ...
                if (idx < 0 || idx >= static_cast<int64_t>(s.size())) return std::string();
        ...
        case node_kind::dict_lit: {
            ...
            for (size_t i = 0; i + 1 < kids.size(); i += 2) {
                ...
                std::string key_s = std::holds_alternative<std::string>(k)
                    ? std::get<std::string>(k)
...
void interpreter::impl::do_assign(node_id target_id, emma_value value, const std::shared_ptr<scope>& scp) {
    ...
    if (k == node_kind::attr) {
        ...
        if (std::holds_alternative<std::shared_ptr<emma_dict>>(obj)) {
            auto& d = std::get<std::shared_ptr<emma_dict>>(obj);
            if (d) d->items[name] = std::move(value);
        }
        return;
    }
...
emma_value interpreter::impl::call_fn(const std::shared_ptr<callable>& fn, std::vector<emma_value> args) {
    // host-implemented?
    if (fn->host_impl) {
        return fn->host_impl(std::move(args));
    }
    // Emma-defined: 构造 call scope 挂 closure
    auto call_scope = std::make_shared<scope>(fn->closure);
    for (size_t i = 0; i < fn->params.size(); ++i) {
        emma_value a = (i < args.size()) ? std::move(args[i]) : emma_value(nullptr);
        call_scope->define(fn->params[i], std::move(a));
    }
    // body 是一个 block wrapper node id
    if (fn->body.empty()) return nullptr;
    node_id body_id = fn->body[0];
    try {
        exec_block(body_id, call_scope);
    } catch (runtime_signal& sig) {
        if (sig.kind == signal_kind::ret) return std::move(sig.value);
        // break/continue 越界: 视作错误但不 hard crash
        return nullptr;
    }
    return nullptr;
}
...
emma_value interpreter::impl::eval_binop(const std::string& op, emma_value left, emma_value right) {
    ...
    if (op == "==" || op == "!=") {
        ...
        } else if (std::holds_alternative<std::nullptr_t>(left) && std::holds_alternative<std::nullptr_t>(right)) {
            eq = true;
    ...
    if (op == "+" || op == "-" || op == "*" || op == "/" || op == "%") {
        if (both_int(left, right) && op != "/") {
            ...
            if (op == "-") return l - r;
            if (op == "*") return l * r;
            if (op == "%") return r == 0 ? static_cast<int64_t>(0) : (l % r);
        }
        ...
        if (op == "%") return r == 0.0 ? 0.0 : std::fmod(l, r);
    }
    // 比较
    if (op == "<" || op == ">" || op == "<=" || op == ">=") {
        if (std::holds_alternative<std::string>(left) && std::holds_alternative<std::string>(right)) {
            ...
            if (op == "<")  return static_cast<bool>(ls < rs);
            if (op == ">")  return static_cast<bool>(ls > rs);
            if (op == "<=") return static_cast<bool>(ls <= rs);
            if (op == ">=") return static_cast<bool>(ls >= rs);
        }
        double l = to_number_double(left);
        double r = to_number_double(right);
        if (op == "<")  return static_cast<bool>(l < r);
        if (op == ">")  return static_cast<bool>(l > r);
        if (op == "<=") return static_cast<bool>(l <= r);
        if (op == ">=") return static_cast<bool>(l >= r);
    }

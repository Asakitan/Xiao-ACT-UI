...
bool controlled_file(const char* root_utf8, size_t root_length,
                     const char* filename) {
...
int controlled_lua_searcher(lua_State* state) {
    ...
    size_t root_length = 0;
    const char* root =
        lua_tolstring(state, lua_upvalueindex(2), &root_length);
...
bool field_denied(const char* value, const char* const* denied,
                  size_t denied_count) {
    if (value == nullptr) return true;
    for (size_t index = 0; index < denied_count; ++index) {
        if (std::strcmp(value, denied[index]) == 0) return true;
    }
    return false;
}

void copy_module(lua_State* state, const char* name, int environment,
                 const char* const* denied, size_t denied_count) {
    lua_getglobal(state, name);
    if (!lua_istable(state, -1)) {
        lua_pop(state, 1);
        return;
...
void save_package_field(lua_State* state, int package_index,
                        const char* field, int& reference) {
...
int configure_package_body(lua_State* state) {
    const auto* data = static_cast<const package_path_data*>(
        lua_touserdata(state, 1));
    if (data->package_ref != LUA_NOREF &&
        data->package_ref != LUA_REFNIL) {
...
int arm_body(lua_State* state) {
    auto* values = static_cast<std::pair<sandbox_state*,
                                        const lua_sandbox_config*>*>(
        lua_touserdata(state, 1));
    ...
    if (lua_istable(state, -1)) {
        ...
        save_package_field(state, package_index, "path",
                           sandbox->original_package_path_ref);
        save_package_field(state, package_index, "cpath",
                           sandbox->original_package_cpath_ref);
    ...
    if (config->allow_coroutine) copy_global(state, "coroutine", environment);
    ...
    static const char* const denied_os[] = {
    ...
    if (config->allow_require) {
        ...
        static const char* const denied_package[] = {
            "cpath", "loadlib", "path", "preload", "searchers",
            "searchpath"};
    ...
    if (config->deny_specific_globals != nullptr) {
        for (size_t index = 0;
             index < config->deny_specific_globals_count; ++index) {
...
void restore_package_ref(lua_State* state, int package_index,
                         const char* field, int reference) {
    if (reference == LUA_NOREF || reference == LUA_REFNIL) return;
...
int restore_body(lua_State* state) {
    ...
    if (sandbox->original_globals_ref != LUA_NOREF &&
        sandbox->original_globals_ref != LUA_REFNIL) {
        lua_rawgeti(state, LUA_REGISTRYINDEX,
                    sandbox->original_globals_ref);
        lua_rawseti(state, LUA_REGISTRYINDEX, LUA_RIDX_GLOBALS);
    }
    lua_getglobal(state, "package");
    if (lua_istable(state, -1)) {
        const int package_index = lua_gettop(state);
        restore_package_ref(state, package_index, "path",
                            sandbox->original_package_path_ref);
        restore_package_ref(state, package_index, "cpath",
                            sandbox->original_package_cpath_ref);
...
extern "C" void* sandbox_allocator(void* user_data, void* pointer,
    ...
    if (new_size == 0) {
        if (pointer != nullptr) {
            sandbox->used_bytes = sandbox->used_bytes >= old_size
                                      ? sandbox->used_bytes - old_size
                                      : 0;
        }
        sandbox->original_allocator(sandbox->original_allocator_data,
                                    pointer, old_size, 0);
        return nullptr;
    }
    uint64_t next = sandbox->used_bytes;
    if (pointer != nullptr) next = next >= old_size ? next - old_size : 0;
    ...
    void* result = sandbox->original_allocator(
        sandbox->original_allocator_data, pointer, old_size, new_size);
    if (result != nullptr) sandbox->used_bytes = next;
    return result;
}
...
int32_t sandbox_set_controlled_package_path_locked(lua_State* state,
                                                    ...
                                                    const char* root,
                                                    size_t root_length) {
    if (state == nullptr || path == nullptr || root == nullptr) {
    ...
    package_path_data data{path, path_length, LUA_NOREF, root, root_length};
    {
        ...
        if (found != g_sandboxes.end()) {
            data.package_ref = found->second->original_package_ref;
        }
    }
    if (protected_trampoline(state, configure_package_body, &data, 0) !=
        LUA_OK) {
...
int32_t sandbox_disarm_locked(lua_State* state) noexcept {
    try {
        std::unique_ptr<sandbox_state> sandbox;
        {
            std::lock_guard map_lock(g_sandboxes_mutex);
            const auto found = g_sandboxes.find(state);
            if (found == g_sandboxes.end()) return SAO_ERR_INVALID_ARGUMENT;
            sandbox = std::move(found->second);
            g_sandboxes.erase(found);
        }
        lua_sethook(state, sandbox->original_hook,
                    sandbox->original_hook_mask,
                    sandbox->original_hook_count);
        if (sandbox->max_bytes != 0) {
            lua_setallocf(state, sandbox->original_allocator,
                          sandbox->original_allocator_data);
        }
        const int restore_status =
            protected_trampoline(state, restore_body, sandbox.get(), 0);
...
sao_plugins_luahost_sandbox_arm(lua_State* state,
                                const lua_sandbox_config* config) {
    ...
    try {
        ...
        sandbox->original_allocator =
            lua_getallocf(state, &sandbox->original_allocator_data);
        ...
        std::pair<sandbox_state*, const lua_sandbox_config*> values{
            sandbox.get(), config};
        if (detail::protected_trampoline(state, arm_body, &values, 0) !=
            LUA_OK) {
            detail::capture_state_error_locked(state, -1);
            lua_settop(state, base);
            (void)detail::protected_trampoline(state, restore_body,
                                               sandbox.get(), 0);
            lua_settop(state, base);
            release_refs(state, *sandbox);
            return SAO_ERR_OS_CALL_FAILED;
        ...
        if (sandbox->max_bytes != 0) {
            const uint64_t kilobytes = static_cast<uint64_t>(
                std::max(lua_gc(state, LUA_GCCOUNT), 0));
            const uint64_t remainder = static_cast<uint64_t>(
                std::max(lua_gc(state, LUA_GCCOUNTB), 0));
        ...
        sandbox_state* installed = sandbox.get();
        {
            ...
            if (!inserted) return SAO_ERR_INVALID_ARGUMENT;
        ...
        if (config->max_instructions_per_run != 0) {
            ...
            const int count = static_cast<int>(
                std::min(config->max_instructions_per_run, maximum));
...
sao_plugins_luahost_sandbox_disarm(lua_State* state) {
    ...
    return status == SAO_OK ? detail::sandbox_disarm_locked(state) : status;
#else
    return SAO_ERR_NOT_IMPLEMENTED;
#endif
}
...
#if defined(SAO_HAS_LUA)
    detail::state_operation operation;
    return detail::acquire_state_operation(state, operation) == SAO_OK &&
           detail::sandbox_is_armed_locked(state);
#else
    return false;
#endif
}

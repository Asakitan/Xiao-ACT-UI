...
(function () {
  ...
  var DETAILS = {
    ...
    q1: {
      ...
      html: '<p>双击安装目录里的 <code>SaoAuto.exe</code>。正常界面启动后播放 Link Start，随后进入主界面；首次使用还会打开本指南。</p><ul><li>屏幕右下角出现 NerveGear 圆形悬浮按钮。</li><li>系统托盘出现 SAO Auto 图标（如果托盘折叠了，先展开隐藏图标）。</li><li>默认使用 <kbd>HOME</kbd> 开关主菜单，可在快捷键面板中改键。</li></ul><p>启动动画不等同于插件或在线 AI 服务已经连接；请以各面板的实际状态为准。</p>'
    },
    q2: {
      section: '01 快速开始', title: '打开主菜单',
      html: '<p>两种方式，效果一样：</p><ul><li>左键点击悬浮按钮。</li><li>按默认的 <kbd>HOME</kbd> 菜单键。</li></ul><p>菜单会围绕悬浮按钮展开；即使覆盖层此前隐藏，菜单键也会重新显示并打开菜单。所有已启用功能和工具都从主菜单的入口页面进。</p>'
    },
    q3: {
    ...
    t3: {
      ...
      html: '<p>快捷键面板显示菜单键组合、状态和操作；默认是 <kbd>HOME</kbd>。点击“录入组合键”再按下新组合，成功后自动保存并显示“已保存”；发生冲突或保存失败时显示相应提示。“恢复默认”恢复菜单键默认组合。</p>'
    },
    t4: {
    ...
    h1: {
      ...
      html: '<p>按一下展开主菜单，再按一下收起。任何程序在前台时都有效。菜单键可在托盘菜单 → 快捷键中重新录入。</p><p>若覆盖层此前已隐藏，按菜单键会重新显示并打开菜单。</p>'
    },
    /* 04 面板和插件 */
    p0: {

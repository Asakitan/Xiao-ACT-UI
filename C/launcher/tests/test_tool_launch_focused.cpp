...
TEST_CASE("AI Editor owner service creates reuses and retires one compositor panel",
          "[launcher][tool_launch][ui_service][compositor][focused]") {
    TempTree tree;
    tree.install_fixture();
    ScopedEnvironment python_path(L"PYTHONPATH", nullptr);
    BoundCompositor compositor;
    const auto windows_before = current_process_top_level_windows();

    sao::launcher::tool_launch::AiEditorProcessOwner owner(tree.root.wstring());
    REQUIRE(owner.open() == SAO_STATUS_OK);
    sao::launcher::tool_launch::AiEditorLaunchSnapshot started;
    REQUIRE(
        wait_for_phase(owner, sao::launcher::tool_launch::AiEditorLaunchPhase::started, started));
    REQUIRE(started.process_id != 0);
    HANDLE child = OpenProcess(SYNCHRONIZE, FALSE, started.process_id);
    REQUIRE(child != nullptr);

    REQUIRE(wait_for_layer_count(owner, compositor.get(), 1));
    CHECK(current_process_top_level_windows() == windows_before);
    REQUIRE(owner.open() == SAO_STATUS_OK);
...
TEST_CASE("AI Editor owner keeps panel ownership after foreign-thread teardown",
          "[launcher][tool_launch][ui_service][owner_thread][focused]") {
    TempTree tree;
    tree.install_fixture();
    ScopedEnvironment python_path(L"PYTHONPATH", nullptr);
    BoundCompositor compositor;
    ...
    std::thread foreign([&] { foreign_status.store(owner.take_offline()); });
    foreign.join();
    CHECK(foreign_status.load() == SAO_STATUS_ERR_ACCESS_DENIED);
    CHECK(compositor_layer_count(compositor.get()) == 1);

    sao::launcher::tool_launch::AiEditorLaunchSnapshot preserved;
    REQUIRE(owner.snapshot(preserved) == SAO_STATUS_OK);
    CHECK(preserved.phase == sao::launcher::tool_launch::AiEditorLaunchPhase::started);
    CHECK(preserved.process_id == started.process_id);
    REQUIRE(owner.service_ui() == SAO_STATUS_OK);
    REQUIRE(owner.open() == SAO_STATUS_OK);
    CHECK(compositor_layer_count(compositor.get()) == 1);
...
TEST_CASE("AI Editor owner reopens after successful offline teardown",
          "[launcher][tool_launch][ui_service][offline][reopen][focused]") {
    TempTree tree;
    tree.install_fixture();
    ScopedEnvironment python_path(L"PYTHONPATH", nullptr);
    BoundCompositor compositor;
    ...
    HANDLE first_child = OpenProcess(SYNCHRONIZE, FALSE, first.process_id);
    REQUIRE(first_child != nullptr);

    REQUIRE(owner.take_offline() == SAO_STATUS_OK);
    CHECK(compositor_layer_count(compositor.get()) == 0);
    CHECK(WaitForSingleObject(first_child, 5000) == WAIT_OBJECT_0);

    REQUIRE(owner.open() == SAO_STATUS_OK);
    sao::launcher::tool_launch::AiEditorLaunchSnapshot second;
    REQUIRE(wait_for_phase(
        owner, sao::launcher::tool_launch::AiEditorLaunchPhase::started, second));
    REQUIRE(second.process_id != 0);
    CHECK(second.process_id != first.process_id);
    CloseHandle(first_child);
...
TEST_CASE("AI Editor owner-thread destructor retires an active panel first",
          "[launcher][tool_launch][ui_service][destructor][owner_thread][focused]") {
    TempTree tree;
    tree.install_fixture();
    ScopedEnvironment python_path(L"PYTHONPATH", nullptr);
    BoundCompositor compositor;
...
TEST_CASE("AI Editor owner service removes the panel after child exit",
          "[launcher][tool_launch][ui_service][child_exit][focused]") {
    TempTree tree;
    tree.install_fixture();
    ScopedEnvironment python_path(L"PYTHONPATH", nullptr);
    BoundCompositor compositor;
    ...
    HANDLE child = OpenProcess(PROCESS_TERMINATE | SYNCHRONIZE, FALSE, started.process_id);
    REQUIRE(child != nullptr);
    REQUIRE(TerminateProcess(child, 27));
    REQUIRE(WaitForSingleObject(child, 5000) == WAIT_OBJECT_0);
    CloseHandle(child);

    REQUIRE(wait_for_layer_count(owner, compositor.get(), 0));
    sao::launcher::tool_launch::AiEditorLaunchSnapshot stopped;
    REQUIRE(owner.snapshot(stopped) == SAO_STATUS_OK);
    CHECK(stopped.phase == sao::launcher::tool_launch::AiEditorLaunchPhase::failed);
    CHECK(stopped.last_status == SAO_STATUS_ERR_PROCESS_GONE);
    CHECK(stopped.has_exit_code);
    CHECK(stopped.exit_code == 27);
    REQUIRE(owner.take_offline() == SAO_STATUS_OK);
}

...
struct TeardownRecorder {
    ...
    static int pollExitImmediately(void*) { return 1; }
};
...
struct CompositionRecorder {
    ...
    static sao_status_t shellShutdown(void* user) {
        ...
        if (self->shell_shutdown_attempt >=
            self->shell_shutdown_statuses.size()) {
    ...
    static sao_status_t licenseShutdown(void* user) {
        ...
        if (self->license_shutdown_attempt >=
            self->license_shutdown_statuses.size()) {
            return SAO_STATUS_OK;
        }
        return self->license_shutdown_statuses[
            self->license_shutdown_attempt++];
    }

    static sao_status_t pluginsDiscover(sao_platform_ctx*,
    ...
    static sao_status_t pluginsActivate(sao_plugins_registry* registry,
                                        void* user) {
        auto* self = static_cast<CompositionRecorder*>(user);
    ...
    static sao_status_t pluginsShutdown(sao_plugins_registry* registry,
                                        void* user) {
        auto* self = static_cast<CompositionRecorder*>(user);
        REQUIRE(registry == reinterpret_cast<sao_plugins_registry*>(self));
        self->steps.emplace_back("plugins_shutdown");
        if (self->plugin_shutdown_attempt >=
            self->plugin_shutdown_statuses.size()) {
            return SAO_STATUS_OK;
        }
        return self->plugin_shutdown_statuses[
            self->plugin_shutdown_attempt++];
    }
    ...
    static sao_status_t securityInit(const sao_security_config* cfg, void* user) {
        ...
        return cfg && cfg->enable_anti_debug && cfg->enable_anti_dump
            ? self->security_status : SAO_STATUS_INVALID_ARGUMENT;
    }
    ...
    static sao_status_t platformBringup(const sao_platform_config* cfg,
                                        sao_platform_ctx** out, void* user) {
        auto* self = static_cast<CompositionRecorder*>(user);
        self->steps.emplace_back("platform_bringup");
        self->platform_log_level = cfg && cfg->log_level
            ? cfg->log_level : "<null>";
    ...
    static sao_status_t platformTeardown(sao_platform_ctx* ctx, void* user) {
        ...
        if (self->platform_teardown_attempt >=
            self->platform_teardown_statuses.size()) {
            return SAO_STATUS_OK;
        }
        return self->platform_teardown_statuses[
            self->platform_teardown_attempt++];
    }
...
struct ConfigFile {
        ...
        ~ConfigFile() { DeleteFileW(path.c_str()); }

        std::wstring argument() const { return L"--config=" + path; }
...
int findStep(const std::vector<std::string>& steps, const char* name) {
    for (size_t i = 0; i < steps.size(); ++i) {
        if (steps[i] == name) return static_cast<int>(i);
    }
    return -1;
}
...
std::string pathUtf8(const std::filesystem::path& path) {
    ...
    REQUIRE(WideCharToMultiByte(
        CP_UTF8, 0, wide.c_str(), -1, utf8.data(), required,
...
struct ProductionRolloutGuard {
    ProductionRolloutGuard() : directory(uniqueRolloutDirectory()),
                               directory_utf8(pathUtf8(directory)) {
        SetEnvironmentVariableW(SAO_DUAL_RUN_ENV_VAR_NAME, nullptr);
...
TEST_CASE("launcher_init_pipeline_run_no_deps_returns_ok",
    ...
    int exit_code = 12345;
    sao_status_t rc = sao_launcher_init_pipeline_run(
        1, argv, &hooks, &exit_code);
...
TEST_CASE("launcher_init_pipeline_teardown_reverse_order",
          "[launcher][init_pipeline][wave5]") {
...
TEST_CASE("launcher_init_pipeline_no_config_keeps_optional_providers_disabled",
    ...
    REQUIRE(sao_launcher_init_pipeline_run(1, argv, &hooks, &exit_code)
        == SAO_STATUS_OK);
    REQUIRE(exit_code == SAO_EXIT_OK);
...
TEST_CASE("launcher raw license transport rejects prevalidated responses",
    ...
    const auto base =
        std::filesystem::path(verified_locally.path).parent_path();
    REQUIRE(sao::launcher::loadLauncherProviderConfiguration(
                base.c_str(), verified_locally.path.c_str()) ==
            SAO_STATUS_OK);
    const auto accepted =
    ...
    REQUIRE(sao::launcher::loadLauncherProviderConfiguration(
                base.c_str(), bypass_attempt.path.c_str()) ==
...
TEST_CASE("launcher_init_pipeline_enabled_provider_missing_fails_closed",
    ...
    REQUIRE(sao_launcher_init_pipeline_run(2, argv, nullptr, &exit_code)
        == SAO_STATUS_INTERNAL);
...
TEST_CASE("launcher_init_pipeline_enabled_shell_provider_missing_fails_closed",
    ...
    DualRunChildGuard dual_run_child;
    ConfigFile config(
        R"json({"shell":{"enabled":true,"metadata":"shell.meta"}})json");
    const auto metadata = std::filesystem::path(config.path).parent_path() /
        L"shell.meta";
    {
    ...
    REQUIRE(sao::launcher::loadLauncherProviderConfiguration(
                std::filesystem::path(config.path).parent_path().c_str(),
                config.path.c_str()) == SAO_STATUS_NOT_IMPLEMENTED);
...
TEST_CASE("launcher_init_pipeline_enabled_plugins_provider_missing_fails_closed",
    ...
    DualRunChildGuard dual_run_child;
    ConfigFile config(
        R"json({"plugins":{"enabled":true,"roots":["plugins"]}})json");
    CompositionRecorder composition;
    CompositionHookGuard composition_guard(composition);
...
TEST_CASE("launcher_platform_config_applies_core_log_filter",
          "[launcher][logging][core][w18]") {
#if defined(SAO_LAUNCHER_CORE_LOG_PROVIDER)
    ...
    REQUIRE(sao::launcher::buildPlatformConfig(
        state, config, log_level, sizeof(log_level)));
    REQUIRE(std::string{config.log_level} == "error");
    REQUIRE(sao_core_log(SAO_LOG_WARN, "launcher.test", "filtered") ==
            SAO_STATUS_OK);
    REQUIRE(callback_calls == 0);
    REQUIRE(sao_core_log(SAO_LOG_ERROR, "launcher.test", "visible") ==
            SAO_STATUS_OK);
...
TEST_CASE("launcher_headless_production_path_applies_rollout_and_persists_anon_id",
    ...
    wchar_t* argv[] = {argv0};
    int exit_code = -1;
    REQUIRE(sao_launcher_init_pipeline_run(1, argv, nullptr, &exit_code)
            == SAO_STATUS_OK);
    ...
    const std::string anon_id{
        std::istreambuf_iterator<char>(anon_input),
        std::istreambuf_iterator<char>()};
    REQUIRE(anon_id.size() >= 32);

    char body[4096]{};
    size_t written = 0;
    REQUIRE(sao_telemetry_test_get_last_body(
        body, sizeof(body), &written) == SAO_OK);
    const std::string captured(body, written);
    REQUIRE(captured.find("sao.rollout.launch_success") !=
            std::string::npos);
#else
...
TEST_CASE("launcher_shutdown_retains_platform_context_until_cleanup_succeeds",
    ...
    for (const sao_status_t failure_status :
         {cleanup_unknown, SAO_STATUS_INTERNAL}) {
        ...
        REQUIRE(state.platform_ctx ==
                reinterpret_cast<sao_platform_ctx*>(&composition));
...
TEST_CASE("launcher_shutdown_retries_plugins_before_lower_subsystems",
    ...
    sao::launcher::AppState state{};
    state.plugins_registry = reinterpret_cast<sao_plugins_registry*>(
        &composition);
    state.shell_active = true;
    state.license_active = true;

    REQUIRE_FALSE(sao::launcher::runFullShutdown(state, false));
    REQUIRE(state.plugins_registry ==
            reinterpret_cast<sao_plugins_registry*>(&composition));
...
TEST_CASE("launcher_single_instance_second_run_fails",
    ...
    wchar_t name[128]{};
    _snwprintf_s(name, 128, _TRUNCATE,
                 L"Global\\SaoAuto.Instance.%016llx", h);

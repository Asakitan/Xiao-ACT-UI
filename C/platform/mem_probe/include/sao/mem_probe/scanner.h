...
// Each scan charges remote reads against one byte/read/deadline budget.
...
// Signed integer dtypes compare as signed values; unsigned integer dtypes
// compare as unsigned values. Float CHANGED/UNCHANGED compare bit patterns;
// float INCREASED/DECREASED reject either operand when it is NaN.
...
enum : uint32_t {
    SAO_MEMPROBE_SCAN_BOUNDS_V2_ABI_VERSION = 2u,
};

typedef struct sao_memprobe_scan_bounds_v2_s {
    uint32_t max_addrs;
...
SAO_CORE_API sao_status_t SAO_CORE_CALL sao_memprobe_scan_first_v2(
    sao_core_process_handle_t process, sao_memprobe_dtype_t dtype, uint64_t value,
...
// AVX2 available flag (runtime detected via CPUID plus OSXSAVE/XGETBV)。
...
SAO_CORE_API sao_status_t SAO_CORE_CALL sao_memprobe_scan_pattern_v2(
    sao_core_process_handle_t process, const uint8_t* pattern, const uint8_t* mask,

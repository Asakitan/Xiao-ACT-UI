...
extern "C" sao_status_t SAO_NET_CALL sao_net_ws_start_receive(
    ...
    if (handle->receive_thread.joinable()) return SAO_STATUS_ERR_ALREADY_EXISTS;
...
extern "C" sao_status_t SAO_NET_CALL sao_net_ws_close(
    ...
    {
        std::lock_guard<std::mutex> guard(handle->send_mutex);
        if (!handle->closed.exchange(true, std::memory_order_acq_rel)) {
            error = WinHttpWebSocketClose(handle->socket.get(), close_code,
                                          nullptr, 0);
        }
    }
    if (handle->receive_thread.joinable() &&
        handle->receive_thread.get_id() != std::this_thread::get_id()) {
        handle->receive_thread.join();
    }
    return error == ERROR_SUCCESS ? SAO_STATUS_OK
                                  : sao::net::internal::map_winhttp_error(error);
}
...
extern "C" void SAO_NET_CALL sao_net_ws_destroy(sao_net_ws_client_handle_t handle) {
    ...
    if (handle->socket && !handle->closed.load(std::memory_order_acquire)) {
        (void)sao_net_ws_close(handle, 1000);
    } else if (handle->receive_thread.joinable()) {
        handle->receive_thread.join();
    }
    delete handle;
}

...
int SAO_ENGINE_CALL wave5_bridge(const char* topic_utf8,
...
sao_sdk_status_t SAO_SDK_CALL event_subscribe(
    ...
    if (sub == nullptr) return SAO_SDK_ERR_NOT_INITIALIZED;
    ...
    const sao_status_t rc = sao_engine_event_bus_subscribe_wave5(
        rt.event_bus, topic_utf8, /*priority=*/0, wave5_bridge, sub,
        &bus_token);
    if (rc != SAO_STATUS_OK) {
        delete sub;
    ...
    if (out_subscription != nullptr) *out_subscription = sub->sdk_token;
    return SAO_SDK_OK;
}

sao_sdk_status_t SAO_SDK_CALL event_unsubscribe(
    ...
    {
        std::lock_guard<std::mutex> lk(state->mu);
        auto it = std::find_if(state->event_subs.begin(),
                                state->event_subs.end(),
                                [subscription](const EventSubscription& s) {
                                    return s.sdk_token == subscription;
                                });
        if (it == state->event_subs.end()) return SAO_SDK_ERR_NOT_FOUND;
        bus_token  = it->bus_token;
        heap_owner = it->heap_owner;
        state->event_subs.erase(it);
    }
...
sao_sdk_status_t SAO_SDK_CALL event_publish(
    void* ctx_impl,
    const char* topic_utf8,
    ...
    auto& rt = SharedRuntime::instance();
    if (rt.event_bus == nullptr) return SAO_SDK_ERR_NOT_INITIALIZED;
    const sao_status_t rc = sao_engine_event_bus_publish_wave5(
        rt.event_bus, topic_utf8, json_payload_utf8, payload_len);
    if (rc != SAO_STATUS_OK) return static_cast<sao_sdk_status_t>(rc);
    return SAO_SDK_OK;
}
...
extern "C" SAO_SDK_API sao_sdk_status_t SAO_SDK_CALL sao_sdk_subscribe_event(
    ...
    return ctx->event->subscribe(ctx->ctx_impl, event_type_utf8, callback,
                                  user_data, out_handle);
}

extern "C" SAO_SDK_API sao_sdk_status_t SAO_SDK_CALL sao_sdk_unsubscribe_event(
    const struct SaoSdkContext* ctx, sao_sdk_subscription_t handle) {
    if (ctx == nullptr) return SAO_SDK_ERR_INVALID_ARGUMENT;
    return ctx->event->unsubscribe(ctx->ctx_impl, handle);
}

extern "C" SAO_SDK_API sao_sdk_status_t SAO_SDK_CALL sao_sdk_publish_event(
    const struct SaoSdkContext* ctx,
    const char* event_type_utf8,
    const uint8_t* data_ptr,
    size_t size) {
    if (ctx == nullptr) return SAO_SDK_ERR_INVALID_ARGUMENT;
    return ctx->event->publish(ctx->ctx_impl, event_type_utf8, data_ptr,
                                size);
}

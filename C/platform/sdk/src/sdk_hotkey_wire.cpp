...
sao_sdk_status_t SAO_SDK_CALL hotkey_register(
    void* ctx_impl,
    ...
    return provider_hotkey_register(state, hotkey_name_utf8, virtual_key,
                                    modifier_mask, callback, user_data,
                                    out_id);
}

sao_sdk_status_t SAO_SDK_CALL hotkey_unregister(
...
size_t fire_hotkey_by_binding_id(ContextState* state, const char* binding_id_utf8) {
    if (state == nullptr || binding_id_utf8 == nullptr) return 0;
    std::vector<HotkeyEntry> snap;
    {
        std::lock_guard<std::mutex> lk(state->mu);
        for (const auto& h : state->hotkeys) {
            if (h.binding_id == binding_id_utf8) snap.push_back(h);
        }
    }
    for (const auto& h : snap) {
        if (h.plugin_cb != nullptr) h.plugin_cb(h.sdk_id, h.plugin_ud);
    }
    return snap.size();
}
...
extern "C" SAO_SDK_API sao_sdk_status_t SAO_SDK_CALL sao_sdk_register_hotkey(
    ...
    return ctx->hotkey->register_hotkey(ctx->ctx_impl, spec->binding_id_utf8,
                                        spec->virtual_key, spec->modifiers,
                                        callback, user_data, out_handle);
}

extern "C" SAO_SDK_API sao_sdk_status_t SAO_SDK_CALL sao_sdk_unregister_hotkey(
    const struct SaoSdkContext* ctx, sao_sdk_hotkey_id_t handle) {
    if (ctx == nullptr || ctx->hotkey == nullptr ||
        ctx->hotkey->unregister_hotkey == nullptr) {
        return SAO_SDK_ERR_INVALID_ARGUMENT;
    }

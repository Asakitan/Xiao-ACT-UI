// SAO Auto - fisheye lens math and stateful animation tests.
//
...
TEST_CASE("fisheye_hover_item_max_size", "[ui][fisheye][runtime]") {
...
TEST_CASE("fisheye_neighbor_items_partially_zoomed", "[ui][fisheye][runtime]") {
    const SaoUiFisheyeConfig& cfg = *sao_ui_fisheye_default_config();
    auto btns = make_column(cfg, 6);
    sao_ui_fisheye_handle_t h = nullptr;
    REQUIRE(sao_ui_fisheye_create(&cfg, &h) == SAO_STATUS_OK);
...
TEST_CASE("fisheye_edge_items_base_size", "[ui][fisheye][runtime]") {
    const SaoUiFisheyeConfig& cfg = *sao_ui_fisheye_default_config();
...
TEST_CASE("fisheye_no_hover_all_base_size", "[ui][fisheye][runtime]") {
    const SaoUiFisheyeConfig& cfg = *sao_ui_fisheye_default_config();
    auto btns = make_column(cfg, 6);
    sao_ui_fisheye_handle_t h = nullptr;
    REQUIRE(sao_ui_fisheye_create(&cfg, &h) == SAO_STATUS_OK);
...
TEST_CASE("fisheye_animate_smooth_transition", "[ui][fisheye][runtime]") {
    const SaoUiFisheyeConfig& cfg = *sao_ui_fisheye_default_config();
    auto btns = make_column(cfg, 6);
    sao_ui_fisheye_handle_t h = nullptr;
    REQUIRE(sao_ui_fisheye_create(&cfg, &h) == SAO_STATUS_OK);

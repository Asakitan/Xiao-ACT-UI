// SAO Auto - platform/ui NOT_IMPLEMENTED classification.
// See ui_gap_support.h for the isolation contract.
...
// return in platform/ui/src as of the static scan.  Adding a
// new row means the gap test-suite counters (matrix_totals()) also
...
std::vector<GapRow> classification_matrix() {
    ...
    // The three GPU-capture CPU functions share a non-Windows stub.
    rows.push_back({
...
GapClass DcompLateBindMock::classify_outcome(bool loaded,
    ...
    // DComp bridge — no gap.  Otherwise the gap taxonomy pins us to

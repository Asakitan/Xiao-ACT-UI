// Tests for overlay_adapter lifecycle, naming, and BGRA presentation.
//
...
// Adapter test-only helpers.  SAO_UI_API forward-decls match the DLL
...
TEST_CASE("adapter_lifecycle_setters", "[ui][adapter][automation]") {
    // No real compositor — pass nullptr.  The adapter fixture path
...
TEST_CASE("adapter_unique_layer_names", "[ui][adapter][automation]") {
...
TEST_CASE("adapter_bgra_presenter_frame", "[ui][adapter][automation]") {
    ...
    // start_fade snaps to target immediately in the fixture path.
...
TEST_CASE("adapter_real_layer_backing_composites_and_releases", "[ui][adapter][layer][automation]") {
    SaoCompositorConfig compositor_config{};

// SAO Auto - focused tests for the platform/ui
...
// sao_ui_gap_support_lib mocks.

#include <catch2/catch_test_macros.hpp>

#include "ui_gap_support.h"
...
using sao::ui::gap::classification_matrix;
...
using sao::ui::gap::DcompLateBindMock;

// =====================================================================
// Section 1 — Class A (capability gate) return-code assertions.
// =====================================================================

TEST_CASE("taxonomy_codes_are_distinct",
          "[ui][gap_closure][taxonomy]") {
...
TEST_CASE("matrix_totals_are_pinned",
          "[ui][gap_closure][taxonomy]") {
    const auto t = matrix_totals();
    // If a future change adds/removes a NOT_IMPLEMENTED site the
    // classification matrix in ui_gap_support.cpp must be
...
TEST_CASE("matrix_covers_every_expected_cpp_file",
          "[ui][gap_closure][taxonomy]") {
...
TEST_CASE("alerts_speak_missing_voice_is_capability_gated",
          "[ui][gap_closure][class_a][alerts]") {
...
TEST_CASE("alerts_sound_stop_gate_semantics",
          "[ui][gap_closure][class_a][alerts]") {
...
TEST_CASE("auto_key_send_key_arg_validation",
          "[ui][gap_closure][class_a][auto_key]") {
...
TEST_CASE("auto_key_get_key_state_arg_and_gate",
          "[ui][gap_closure][class_a][auto_key]") {
    bool state = true;
...
TEST_CASE("dcomp_bridge_create_null_out_is_arg_error",
          "[ui][gap_closure][class_a][dcomp]") {
...
TEST_CASE("dcomp_bridge_create_null_config_capability_gated",
          "[ui][gap_closure][class_a][dcomp]") {
...
TEST_CASE("dxgi_dup_create_null_out_is_arg_error",
          "[ui][gap_closure][class_a][dxgi_dup]") {
    sao_status_t rc = sao_ui_dxgi_dup_create(nullptr, nullptr);
    CHECK(rc == SAO_STATUS_ERR_INVALID_ARGUMENT);
}

TEST_CASE("dxgi_dup_release_frame_handle_invalid",
          "[ui][gap_closure][class_a][dxgi_dup]") {
...
TEST_CASE("gpu_capture_supported_is_bool",
          "[ui][gap_closure][class_a][gpu_capture]") {
...
TEST_CASE("gpu_capture_create_null_out_arg_error",
          "[ui][gap_closure][class_a][gpu_capture]") {
    sao_status_t rc = sao_ui_gpu_capture_create(nullptr, nullptr);
    CHECK(rc == SAO_STATUS_ERR_INVALID_ARGUMENT);
}

TEST_CASE("gpu_capture_bgra_from_texture_null_texture",
          "[ui][gap_closure][class_a][gpu_capture]") {
...
TEST_CASE("gpu_capture_ensure_session_handle_invalid",
          "[ui][gap_closure][class_a][gpu_capture]") {
    sao_status_t rc = sao_ui_gpu_capture_ensure_session(nullptr);
    CHECK(rc == SAO_STATUS_ERR_HANDLE_INVALID);
}

TEST_CASE("input_install_ll_hooks_handle_invalid",
          "[ui][gap_closure][class_a][input]") {
...
TEST_CASE("input_uninstall_ll_hooks_handle_invalid",
          "[ui][gap_closure][class_a][input]") {
...
TEST_CASE("send_input_ledger_records_press_release",
          "[ui][gap_closure][mock][auto_key]") {
...
TEST_CASE("shell_notify_ledger_counts_all_ops",
          "[ui][gap_closure][mock][alerts]") {
...
TEST_CASE("dcomp_late_bind_mock_classification",
          "[ui][gap_closure][mock][dcomp]") {
...
TEST_CASE("matrix_class_a_rows_reference_real_files",
          "[ui][gap_closure][taxonomy]") {
...
TEST_CASE("matrix_class_c_rows_reference_real_files",
          "[ui][gap_closure][taxonomy]") {
    const auto rows = classification_matrix();
    uint32_t verified = 0;
    for (const auto& r : rows) {

// SAO Auto — Wave2 flat-token theme tests (G3.1 gate).
//
...
TEST_CASE("theme_get_color_dark_returns_expected", "[ui][theme][wave2]") {
...
TEST_CASE("theme_get_color_light_all_populated", "[ui][theme][wave2]") {
...
TEST_CASE("theme_get_color_glass_all_populated", "[ui][theme][wave2]") {
    for (int32_t i = 0; i < SAO_UI_COLOR_TOKEN_COUNT; ++i) {
        SaoColorRgba rgba{};
        const auto rc = sao_ui_theme_get_color_by_id(
...
TEST_CASE("theme_get_metric_returns_valid", "[ui][theme][wave2]") {
...
TEST_CASE("theme_set_get_active_roundtrip", "[ui][theme][wave2]") {
...
TEST_CASE("theme_invalid_token_returns_error", "[ui][theme][wave2]") {
...
TEST_CASE("theme_invalid_theme_id_returns_error", "[ui][theme][wave2]") {
    SaoColorRgba rgba{};
    int32_t value = 0;
...
TEST_CASE("theme_change_callback_fires_on_set_active", "[ui][theme][wave2]") {
...
TEST_CASE("theme_token_name_returns_nonempty", "[ui][theme][wave2]") {
    struct NameProbe {
        SaoUiColorToken token;
        const char* expected;
    };
...
TEST_CASE("theme_static_assert_size_correctness", "[ui][theme][wave2]") {
    ...
    // (Wave2 flat-token) and both encode the same colour.
    for (int32_t i = 0; i < SAO_UI_COLOR_TOKEN_COUNT; ++i) {
        SaoColorRgba rgba{};
        REQUIRE(sao_ui_theme_get_color_by_id(
                    SAO_UI_THEME_DARK,

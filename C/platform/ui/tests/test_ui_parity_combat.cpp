// Combat-panel Python-authority fixtures cross the production compositor.
...
std::filesystem::path fixture_path(const std::string& name, const char* extension) {
    return std::filesystem::path(SAO_UI_TEST_FIXTURE_DIR) / "combat_parity" /
        (name + extension);
}
...
void require_metadata_format_authority(const CombatFixture& fixture) {
    ...
    REQUIRE(metadata.at("schema") == "sao.ui.python-authority.combat.v1");
...
TEST_CASE("ui parity combat DPS authority transit and formatting",
          "[ui][parity][combat][ui_parity]") {
    require_exact_transit("dps_idle");
    require_exact_transit("dps_active");
}

TEST_CASE("ui parity combat BossHP authority transit and formatting",
          "[ui][parity][combat][ui_parity]") {
    require_exact_transit("bosshp_idle");
    require_exact_transit("bosshp_active");
}

TEST_CASE("ui parity combat HP authority transit and formatting",
          "[ui][parity][combat][ui_parity]") {
    require_exact_transit("hp_idle");
    require_exact_transit("hp_active");
}

TEST_CASE("ui parity combat production compositor preserves metadata z-order",
          "[ui][parity][combat][ui_parity][z-order]") {

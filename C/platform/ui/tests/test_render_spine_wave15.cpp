// Wave 15.2 render-spine tests: pure layer ownership plus real D3D/DComp
...
struct HiddenWindow {
    ...
    ATOM atom = 0;
    const wchar_t* class_name = L"SaoWave152RenderSpine";
...
TEST_CASE("render_spine_layer_bgra_snapshot_is_headless",
          "[ui][render_spine][wave15][pure]") {
    SaoCompositorConfig config = compositor_config();
    sao_ui_compositor_handle_t compositor = nullptr;
    REQUIRE(sao_ui_compositor_create(nullptr, &config, &compositor) == SAO_STATUS_OK);

    SaoLayerConfig layer_definition = layer_config("wave15_headless");
    sao_ui_layer_handle_t layer = nullptr;
...
TEST_CASE("render_spine_compositor_destroy_is_render_thread_affine",
          "[ui][render_spine][wave15][lifecycle]") {
    SaoCompositorConfig config = compositor_config();
    sao_ui_compositor_handle_t compositor = nullptr;
    REQUIRE(sao_ui_compositor_create(nullptr, &config, &compositor) ==
            SAO_STATUS_OK);

    SaoLayerConfig definition = layer_config("wave15_owner_destroy");
    sao_ui_layer_handle_t layer = nullptr;
    ...
    TEST_CASE("render_spine_callback_destroy_is_ignored_during_present",
        ...
        SaoLayerConfig definition = layer_config("wave15_callback_destroy");
        sao_ui_layer_handle_t layer = nullptr;
        REQUIRE(sao_ui_layer_create(compositor, &definition, &layer) ==
            SAO_STATUS_OK);
...
TEST_CASE("render_spine_layer_production_controls_are_headless_safe",
    ...
    SaoLayerConfig layer_definition = layer_config("wave15_controls");
    layer_definition.click_through = false;
    sao_ui_layer_handle_t layer = nullptr;
    REQUIRE(sao_ui_layer_create(compositor, &layer_definition, &layer) == SAO_STATUS_OK);

    RenderLog log{};
    REQUIRE(sao_ui_layer_set_mmf_source(layer, "wave15_optional_mmf") == SAO_STATUS_OK);
...
TEST_CASE("render_spine_callbacks_isolate_exceptions_and_continue",
          "[ui][render_spine][wave15][callback]") {
    SaoCompositorConfig config = compositor_config();
    sao_ui_compositor_handle_t compositor = nullptr;
    REQUIRE(sao_ui_compositor_create(nullptr, &config, &compositor) ==
            SAO_STATUS_OK);
    ...
    for (size_t index = 0; index < layers.size(); ++index) {
        const std::string name =
            "wave15_callback_" + std::to_string(index);
    ...
    TEST_CASE("render_spine_second_fade_reports_interrupted_callback",
        ...
        SaoLayerConfig definition = layer_config("wave15_fade_interrupt");
        sao_ui_layer_handle_t layer = nullptr;
        REQUIRE(sao_ui_layer_create(compositor, &definition, &layer) ==
            SAO_STATUS_OK);
        RenderLog first{};
...
TEST_CASE("render_spine_geometry_updates_are_atomic_and_bounded",
          "[ui][render_spine][wave15][geometry]") {
    SaoCompositorConfig config = compositor_config();
    sao_ui_compositor_handle_t compositor = nullptr;
    REQUIRE(sao_ui_compositor_create(nullptr, &config, &compositor) ==
            SAO_STATUS_OK);

    SaoLayerConfig definition = layer_config("wave15_geometry_guard");
    definition.width = 4;
    definition.height = 4;
    sao_ui_layer_handle_t layer = nullptr;
    REQUIRE(sao_ui_layer_create(compositor, &definition, &layer) ==
    ...
    TEST_CASE("render_spine_hidden_render_callback_retries_after_exception",
        ...
        SaoLayerConfig definition = layer_config("wave15_hidden_retry");
        sao_ui_layer_handle_t layer = nullptr;
        REQUIRE(sao_ui_layer_create(compositor, &definition, &layer) ==
            SAO_STATUS_OK);
        RenderLog log{};
...
TEST_CASE("render_spine_transparent_snapshot_keeps_geometry",
          "[ui][render_spine][wave15][transparent]") {
    SaoCompositorConfig config = compositor_config();
    sao_ui_compositor_handle_t compositor = nullptr;
    REQUIRE(sao_ui_compositor_create(nullptr, &config, &compositor) ==
            SAO_STATUS_OK);

    SaoLayerConfig definition = layer_config("wave15_transparent_geometry");
    definition.width = 3;
    definition.height = 2;
    sao_ui_layer_handle_t layer = nullptr;
    REQUIRE(sao_ui_layer_create(compositor, &definition, &layer) ==
    ...
    TEST_CASE("render_spine_shared_source_clear_drops_cached_pixels",
          "[ui][render_spine][wave15][source]") {
        SaoCompositorConfig config = compositor_config();
        sao_ui_compositor_handle_t compositor = nullptr;
        REQUIRE(sao_ui_compositor_create(nullptr, &config, &compositor) ==
            SAO_STATUS_OK);

        SaoLayerConfig definition = layer_config("wave15_shared_clear");
        sao_ui_layer_handle_t layer = nullptr;
        REQUIRE(sao_ui_layer_create(compositor, &definition, &layer) ==
            SAO_STATUS_OK);
...
TEST_CASE("render_spine_snapshot_uses_production_composition_order",
          "[ui][render_spine][wave15][pixel]") {
    SaoCompositorConfig config = compositor_config();
    sao_ui_compositor_handle_t compositor = nullptr;
    REQUIRE(sao_ui_compositor_create(nullptr, &config, &compositor) == SAO_STATUS_OK);

    SaoLayerConfig base_definition = layer_config("wave15_snapshot_base");
    ...
    SaoLayerConfig overlay_definition = layer_config("wave15_snapshot_overlay");
    ...
    TEST_CASE("render_spine_mmf_source_clear_drops_cached_pixels",
        ...
        SaoLayerConfig definition = layer_config("wave15_mmf_clear");
        sao_ui_layer_handle_t layer = nullptr;
        REQUIRE(sao_ui_layer_create(compositor, &definition, &layer) ==
            SAO_STATUS_OK);
        const std::array<uint8_t, 4> pixel{0x20, 0x10, 0x08, 0x40};
...
TEST_CASE("render_spine_python_bgra_transport_matrix_is_lossless",
          "[ui][render_spine][wave15][bridge][pixel]") {
    SaoCompositorConfig config = compositor_config();
    sao_ui_compositor_handle_t compositor = nullptr;
    REQUIRE(sao_ui_compositor_create(nullptr, &config, &compositor) == SAO_STATUS_OK);
    ...
    for (size_t index = 0; index < fixture_frames.size(); ++index) {
        const std::string name = "wave15_python_fixture_" + std::to_string(index);
        SaoLayerConfig definition = layer_config(name.c_str());
        definition.width = 2;
        definition.height = 1;
...
TEST_CASE("render_spine_authoritative_bgra_fixture_crosses_production_bridge",
          "[ui][render_spine][wave15][bridge][fixture][pixel]") {
    const std::vector<uint8_t> source_pixels =
        read_fixture("wave15_nervegear_hover.bgra");
    ...
    SaoLayerConfig definition = layer_config("wave15_authoritative_frame");
    definition.width = 72;
...
TEST_CASE("render_spine_second_authoritative_fixture_crosses_production_bridge",
          "[ui][render_spine][wave15][bridge][fixture][pixel]") {
    const std::vector<uint8_t> source_pixels = read_fixture("wave15_popup.bgra");
    ...
    SaoLayerConfig definition = layer_config("wave15_second_authoritative_frame");
...
TEST_CASE("render_spine_shared_device_resources_or_skip",
          "[ui][render_spine][wave15][resource]") {
...
TEST_CASE("render_spine_windows_resources_skipped", "[ui][render_spine][wave15][resource]") {

...
nlohmann::json read_metadata() {
    const std::filesystem::path path =
        std::filesystem::path(SAO_UI_TEST_FIXTURE_DIR) /
        "w19_entity_parity.json";
...
std::vector<uint8_t> read_pixels(const std::string& filename) {
    const std::filesystem::path path =
        std::filesystem::path(SAO_UI_TEST_ASSET_DIR) / filename;
...
std::string sha256_hex(const std::vector<uint8_t>& bytes) {
    ...
    REQUIRE(BCryptOpenAlgorithmProvider(
                &algorithm, BCRYPT_SHA256_ALGORITHM, nullptr, 0) == 0);
    DWORD object_bytes = 0;
    DWORD result_bytes = 0;
    REQUIRE(BCryptGetProperty(
...
const nlohmann::json& find_frame(const nlohmann::json& metadata,
                                 std::string_view name) {
    const auto& frames = metadata.at("frames");
    const auto found = std::find_if(
        frames.begin(), frames.end(), [name](const nlohmann::json& frame) {
...
struct ShellFixture {
    ...
    void refresh() {
        REQUIRE(sao_ui_entity_shell_get_snapshot(shell, &snapshot) ==
                SAO_STATUS_OK);
    }
    ...
    std::vector<uint8_t> pixels(uint32_t* width_out = nullptr,
        ...
        REQUIRE(sao_ui_entity_shell_snapshot_bgra(
                    shell, nullptr, 0, &width, &height, &required) ==
                SAO_STATUS_ERR_BUFFER_TOO_SMALL);
        std::vector<uint8_t> result(required);
        REQUIRE(sao_ui_entity_shell_snapshot_bgra(
                    shell, result.data(), result.size(), &width, &height,
    ...
    int32_t center_x() const {
        return snapshot.origin_x + snapshot.nervegear_x +
               SAO_UI_NERVEGEAR_SIZE / 2;
    }

    int32_t center_y() const {
        return snapshot.origin_y + snapshot.nervegear_y +
               SAO_UI_NERVEGEAR_SIZE / 2;
    }
};

void require_authority_crop(const ShellFixture& fixture,
    ...
    REQUIRE(expected.size() == frame.at("bytes").get<size_t>());
        REQUIRE(expected.size() ==
            static_cast<size_t>(frame_width) * frame_height * 4U);
    ...
    for (uint32_t row = 0; row < frame_height; ++row) {
        const auto source = expected.begin() +
            static_cast<size_t>(row) * frame_stride;
        const auto target = composed.begin() +
            (static_cast<size_t>(fixture.snapshot.nervegear_y + row) *
                 composed_width +
...
uint64_t menu_alpha_sum(const ShellFixture& fixture) {
    ...
    REQUIRE(static_cast<uint32_t>(fixture.snapshot.menu_x +
                                  fixture.snapshot.menu_width) <=
            composed_width);
    REQUIRE(static_cast<uint32_t>(fixture.snapshot.menu_y +
                                  fixture.snapshot.menu_height) <=
            composed_height);
    uint64_t total = 0;
    for (int32_t row = 0; row < fixture.snapshot.menu_height; ++row) {
        for (int32_t column = 0; column < fixture.snapshot.menu_width;
             ++column) {
            const size_t alpha_index =
                (static_cast<size_t>(fixture.snapshot.menu_y + row) *
                     composed_width +
...
sao_ui_menu_handle_t make_menu() {
    ...
    REQUIRE(sao_ui_menu_create(nullptr, nullptr,
                               SAO_UI_MENU_MODE_VERTICAL_STRIP, &menu) ==
            SAO_STATUS_OK);
...
void require_phase_checkpoints(sao_ui_menu_handle_t menu,
                               const nlohmann::json& phase_fixture) {
    const auto& checkpoints = phase_fixture.at("checkpoints");
    const int32_t duration_ms =
        phase_fixture.at("duration_ms").get<int32_t>();
    REQUIRE(checkpoints.back().at("elapsed_ms").get<int32_t>() ==
            duration_ms);
    ...
    for (const auto& checkpoint : checkpoints) {
        ...
        REQUIRE(sao_ui_menu_tick(menu, checkpoint_ms - elapsed_ms) ==
                SAO_STATUS_OK);
        ...
        float progress = -1.0F;
        REQUIRE(sao_ui_menu_get_transition_progress(menu, &progress) ==
                SAO_STATUS_OK);
        CHECK(progress == Catch::Approx(
                              checkpoint.at("alpha").get<float>())
                              .margin(0.00001F));
          CHECK(checkpoint.at("done").get<bool>() ==
              (phase == SAO_UI_MENU_PHASE_OPEN ||
...
TEST_CASE("W19 Entity NerveGear frames match Python authority exactly",
    ...
    REQUIRE(metadata.at("schema") ==
            "sao.ui.python-authority.w19.entity.v1");
...
TEST_CASE("W19 Entity menu phase follows Python fade authority",
    ...
    require_phase_checkpoints(menu,
                              metadata.at("phase").at("opening"));
    REQUIRE(sao_ui_menu_hide(menu) == SAO_STATUS_OK);
    require_phase_checkpoints(menu,
                              metadata.at("phase").at("closing"));
    ...
    for (size_t index = 0; index < steps.size(); ++index) {
        if (index > 0) {
            ...
            const int32_t screen_x =
                fixture.snapshot.origin_x + fixture.snapshot.nervegear_x +
                event.at("x").get<int32_t>();
            const int32_t screen_y =
                fixture.snapshot.origin_y + fixture.snapshot.nervegear_y +
                event.at("y").get<int32_t>();
            REQUIRE(sao_ui_entity_shell_handle_mouse(
                        fixture.shell, message, screen_x, screen_y, button,
                        0) == SAO_STATUS_OK);
            fixture.refresh();
        ...
        CHECK(fixture.snapshot.menu_visible ==
              expected.at("menu_visible").get<bool>());
...
TEST_CASE("W19 Entity menu layer consumes Python fade progress",
    ...
    REQUIRE(sao_ui_entity_shell_handle_mouse(
                ...
                fixture.center_y(), 0, 0) == SAO_STATUS_OK);
    REQUIRE(sao_ui_entity_shell_handle_mouse(
                fixture.shell, kLeftButtonUp, fixture.center_x(),
                fixture.center_y(), 0, 0) == SAO_STATUS_OK);
    fixture.refresh();

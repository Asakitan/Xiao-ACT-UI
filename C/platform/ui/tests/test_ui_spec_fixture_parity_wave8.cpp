// Wave 8 / Agent c - ui_spec normalizer fixture parity, Phase 7 deep dive.
//
...
TEST_CASE("fixture_parity_wave8 ui_spec -normal_flat_panel_text_bar",
          "[ui][ui_spec][fixture][wave8]") {
    verify_ui_spec_fixture("normal_flat_panel_text_bar.json");
}

TEST_CASE("fixture_parity_wave8 ui_spec -normal_button_row",
          "[ui][ui_spec][fixture][wave8]") {
    verify_ui_spec_fixture("normal_button_row.json");
}

TEST_CASE("fixture_parity_wave8 ui_spec -normal_table_with_highlight",
          "[ui][ui_spec][fixture][wave8]") {
    verify_ui_spec_fixture("normal_table_with_highlight.json");
}

TEST_CASE("fixture_parity_wave8 ui_spec -nested_deep_sections",
          "[ui][ui_spec][fixture][wave8]") {
    verify_ui_spec_fixture("nested_deep_sections.json");
}

TEST_CASE("fixture_parity_wave8 ui_spec -mutation_unknown_types_dropped",
          "[ui][ui_spec][fixture][wave8]") {
    verify_ui_spec_fixture("mutation_unknown_types_dropped.json");
}

TEST_CASE("fixture_parity_wave8 ui_spec -mutation_text_clamp",
          "[ui][ui_spec][fixture][wave8]") {
    verify_ui_spec_fixture("mutation_text_clamp.json");
}

TEST_CASE("fixture_parity_wave8 ui_spec -invalid_ref_wrong_scalar_types",
          "[ui][ui_spec][fixture][wave8]") {
    verify_ui_spec_fixture("invalid_ref_wrong_scalar_types.json");
}

TEST_CASE("fixture_parity_wave8 ui_spec -invalid_ref_bare_list",
          "[ui][ui_spec][fixture][wave8]") {
...
// wave8c slice enough assert coverage that any regression in the
// helpers (s_clamp / clamp01 / choice / ci / cpos / cz) surfaces
// during CI even if a new fixture doesn't happen to trip it.

TEST_CASE("fixture_parity_wave8 ui_spec normalizer -version endpoint",
          "[ui][ui_spec][wave8]") {
    REQUIRE(sao_engine_ui_spec_version() == SAO_UI_SPEC_VERSION);
    REQUIRE(SAO_UI_SPEC_VERSION == 1);
}

TEST_CASE("fixture_parity_wave8 ui_spec normalizer -malformed input rejected",
          "[ui][ui_spec][wave8]") {
...
TEST_CASE("fixture_parity_wave8 ui_spec normalizer -out buffer too small",
          "[ui][ui_spec][wave8]") {

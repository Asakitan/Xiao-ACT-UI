// Wave 15 Python-authority parity for Alert, BuffMon, and SkillFX.
...
TEST_CASE("wave15 Python effects fixtures retain premultiplied geometry and event authority",
          "[ui][parity][effects][wave15]") {
    Fixture alert = load_fixture("wave15_alert");
    Fixture buffmon = load_fixture("wave15_buffmon_base");
    Fixture skillfx_caption = load_fixture("wave15_skillfx_caption");
    ...
    Fixture skillfx_sdf = load_fixture("wave15_skillfx_sdf_gl");
    if (skillfx_sdf.metadata.value("headless_status", "") == "skip") {
        REQUIRE_FALSE(std::filesystem::exists(fixture_path("wave15_skillfx_sdf_gl", ".bgra")));
...
TEST_CASE("wave15 effects cross the production compositor with exact alpha z order and transit",
          "[ui][parity][effects][wave15][compositor]") {
    Fixture alert = load_fixture("wave15_alert");
    Fixture buffmon = load_fixture("wave15_buffmon_base");
    Fixture skillfx_caption = load_fixture("wave15_skillfx_caption");
...
TEST_CASE("wave15 SkillFX SDF GL authority crosses the production compositor or declares its live gate",
          "[ui][parity][effects][wave15][skillfx][compositor]") {
    Fixture skillfx_sdf = load_fixture("wave15_skillfx_sdf_gl");
    if (skillfx_sdf.metadata.value("headless_status", "") == "skip") {
        REQUIRE(skillfx_sdf.metadata.at("headless_skip").at("required_live_gate") ==
                "real D3D/GL presentation validation");

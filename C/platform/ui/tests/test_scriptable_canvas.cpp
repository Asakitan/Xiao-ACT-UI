// SAO Auto — scriptable canvas op-stream and bitmap-registry tests.
//
// Verifies:
...
//     required coverage expands to 17 shortcut
...
TEST_CASE("canvas_draw_line_records_op", "[ui][scriptable_canvas][runtime]") {
    auto canvas = make_canvas();
    REQUIRE(sao_ui_script_canvas_begin_draw(canvas) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_draw_line(canvas, 12, 34, 56, 78) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_end_draw(canvas) == SAO_STATUS_OK);

    SaoUiCanvasOp buf[4]{};
    size_t written = 0;
    REQUIRE(sao_ui_script_canvas_snapshot_ops(canvas, buf, 4, &written) == SAO_STATUS_OK);
    REQUIRE(written == 1);
...
TEST_CASE("canvas_all_17_ops_record", "[ui][scriptable_canvas][runtime]") {
    // Exercise the required 17-op coverage:
    ...
    REQUIRE(sao_ui_script_canvas_draw_line(canvas, 0, 0, 10, 10) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_draw_rect(canvas, 0, 0, 5, 5) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_draw_rounded_rect(canvas, 0, 0, 5, 5, 2.f) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_draw_oval(canvas, 0, 0, 5, 5) == SAO_STATUS_OK);

    const int32_t verts[] = {0, 0, 10, 0, 10, 10, 0, 10};
    REQUIRE(sao_ui_script_canvas_draw_polygon(canvas, verts, 4) == SAO_STATUS_OK);

    REQUIRE(sao_ui_script_canvas_draw_text(canvas, 0, 0, "hi", 0, 12) == SAO_STATUS_OK);

    // Register bitmap first so draw_bitmap has a valid id.
    const uint32_t px[] = {0xFFFFFFFFu, 0x80808080u, 0x00000000u, 0xFFFFFFFFu};
    int32_t bmp_id = 0;
    REQUIRE(sao_ui_script_canvas_register_bitmap(canvas, px, 2, 2, 8, &bmp_id) == SAO_STATUS_OK);
    REQUIRE(bmp_id != 0);
    REQUIRE(sao_ui_script_canvas_draw_bitmap(canvas, bmp_id, 0, 0, 32, 32) == SAO_STATUS_OK);
    ...
    REQUIRE(sao_ui_script_canvas_set_stroke_color(canvas, 0xFFFF0000u) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_set_fill_color(canvas, 0xFF00FF00u) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_set_line_width(canvas, 2.5f) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_set_font(canvas, 0, 12) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_set_opacity(canvas, 0.75f) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_set_blend(canvas, SAO_UI_CANVAS_BLEND_ADDITIVE) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_translate(canvas, 1.f, 2.f) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_scale(canvas, 1.5f, 1.5f) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_rotate_deg(canvas, 45.f) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_push_state(canvas) == SAO_STATUS_OK);
    ...
    size_t written = 0;
    REQUIRE(sao_ui_script_canvas_snapshot_ops(canvas, snapshot, 24, &written) == SAO_STATUS_OK);
    REQUIRE(written == 17);

    // Build set of recorded op kinds; assert every expected op kind
    // shows up exactly once.
    const int32_t expected[] = {
        SAO_UI_CANVAS_OP_LINE,       SAO_UI_CANVAS_OP_RECT,       SAO_UI_CANVAS_OP_ROUNDED_RECT,
        SAO_UI_CANVAS_OP_OVAL,       SAO_UI_CANVAS_OP_POLYGON,    SAO_UI_CANVAS_OP_TEXT,
        SAO_UI_CANVAS_OP_BITMAP,     SAO_UI_CANVAS_OP_SET_STROKE, SAO_UI_CANVAS_OP_SET_FILL,
        SAO_UI_CANVAS_OP_SET_LINE_W, SAO_UI_CANVAS_OP_SET_FONT,   SAO_UI_CANVAS_OP_SET_OPACITY,
        SAO_UI_CANVAS_OP_SET_BLEND,  SAO_UI_CANVAS_OP_TRANSLATE,  SAO_UI_CANVAS_OP_SCALE,
        SAO_UI_CANVAS_OP_ROTATE,     SAO_UI_CANVAS_OP_PUSH_STATE,
    };
    static_assert(sizeof(expected) / sizeof(expected[0]) == 17, "scriptable canvas covers 17 op kinds");

    for (int32_t want : expected) {
        int count = 0;
        for (size_t i = 0; i < written; ++i) {
            if (snapshot[i].op == want)
                ++count;
        }
...
TEST_CASE("canvas_bitmap_register_lookup", "[ui][scriptable_canvas][runtime]") {
    auto canvas = make_canvas();

    REQUIRE(sao_ui_script_canvas_bitmap_count(canvas) == 0);

    const uint32_t px_a[] = {0xFFFFFFFFu, 0xFF808080u, 0xFF404040u, 0xFF202020u};
    int32_t id_a = 0;
    REQUIRE(sao_ui_script_canvas_register_bitmap(canvas, px_a, 2, 2, 8, &id_a) == SAO_STATUS_OK);
    REQUIRE(id_a != 0);
    ...
    const uint32_t px_b[] = {0xFF000000u, 0xFF111111u, 0xFF222222u, 0xFF333333u};
    REQUIRE(sao_ui_script_canvas_update_bitmap(canvas, id_a, px_b, 2, 2, 8) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_has_bitmap(canvas, id_a));
    REQUIRE(sao_ui_script_canvas_bitmap_count(canvas) == 1);

    // Second bitmap gets a distinct id.
    int32_t id_b = 0;
    REQUIRE(sao_ui_script_canvas_register_bitmap(canvas, px_a, 2, 2, 8, &id_b) == SAO_STATUS_OK);
    REQUIRE(id_b != 0);
    REQUIRE(id_b != id_a);
    REQUIRE(sao_ui_script_canvas_bitmap_count(canvas) == 2);

    // Unregister the first bitmap only.
    REQUIRE(sao_ui_script_canvas_unregister_bitmap(canvas, id_a) == SAO_STATUS_OK);
    ...
    REQUIRE(sao_ui_script_canvas_unregister_bitmap(canvas, id_a) == SAO_STATUS_ERR_NOT_FOUND);

    sao_ui_script_canvas_destroy(canvas);
}

TEST_CASE("canvas_push_pop_clip_stack", "[ui][scriptable_canvas][runtime]") {
    auto canvas = make_canvas();
    REQUIRE(sao_ui_script_canvas_begin_draw(canvas) == SAO_STATUS_OK);

    // Push clip, draw, pop clip, draw again — records four ops in
    // documented order.
    REQUIRE(sao_ui_script_canvas_set_clip(canvas, 4, 4, 100, 100) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_draw_rect(canvas, 8, 8, 32, 32) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_clear_clip(canvas) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_draw_rect(canvas, 0, 0, 200, 200) == SAO_STATUS_OK);
    ...
    SaoUiCanvasOp buf[8]{};
    size_t written = 0;
    REQUIRE(sao_ui_script_canvas_snapshot_ops(canvas, buf, 8, &written) == SAO_STATUS_OK);
    REQUIRE(written == 4);
...
TEST_CASE("canvas_uses_unified_widget_kind_paint_and_destroy",
          "[ui][scriptable_canvas][runtime][lifecycle]") {
    const auto spec = default_spec();
    sao_ui_widget_handle_t widget = nullptr;
    sao_ui_script_canvas_handle_t canvas = nullptr;
    REQUIRE(sao_ui_script_canvas_create(nullptr, &spec, &widget, &canvas) == SAO_STATUS_OK);
    ...
    REQUIRE(sao_ui_script_canvas_draw_rect(canvas, 0, 0, 16, 16) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_end_draw(canvas) == SAO_STATUS_OK);
    ...
    REQUIRE(sao_ui_widget_paint_at(widget, context, 0, 0, 32, 32, 1.0F) == SAO_STATUS_OK);
    ...
    REQUIRE(sao_ui_offscreen_raster_snapshot(raster, pixels.data(), pixels.size(), &bytes, &width,
                                             &height, &stride) == SAO_STATUS_OK);
    REQUIRE(width == 32);
    REQUIRE(height == 32);
    CHECK(pixels[0] == 0);
    CHECK(pixels[1] == 0);
    ...
    CHECK(sao_ui_widget_paint(widget, nullptr, 0, 0, 1, 1) == SAO_STATUS_ERR_INVALID_ARGUMENT);
...
TEST_CASE("canvas_snapshot_aux_survives_pending_vector_relocation",
          "[ui][scriptable_canvas][runtime][hardening][snapshot]") {
...
TEST_CASE("canvas_submit_ops_rebinds_all_aux_and_is_transactional",
    ...
    const int32_t polygon_a[] = {1, 1, 2, 2, 3, 3};
    const int32_t polygon_b[] = {11, 12, 21, 22, 31, 32};
    const std::string text_a = "first";
    const std::string text_b = "second";
    std::array<SaoUiCanvasOp, 4> batch{};
    batch[0].op = SAO_UI_CANVAS_OP_POLYGON;
    batch[0].i[0] = 3;
    batch[0].aux = polygon_a;
    batch[0].aux_len = sizeof(polygon_a);
    batch[1].op = SAO_UI_CANVAS_OP_TEXT;
    batch[1].aux = text_a.data();
    batch[1].aux_len = text_a.size();
    batch[2].op = SAO_UI_CANVAS_OP_POLYGON;
    batch[2].i[0] = 3;
    batch[2].aux = polygon_b;
    batch[2].aux_len = sizeof(polygon_b);
    batch[3].op = SAO_UI_CANVAS_OP_TEXT;
    batch[3].aux = text_b.data();
    batch[3].aux_len = text_b.size();
    REQUIRE(sao_ui_script_canvas_submit_ops(canvas, batch.data(), batch.size()) == SAO_STATUS_OK);

    for (int index = 0; index < 128; ++index) {
        REQUIRE(sao_ui_script_canvas_draw_rect(canvas, index, index, 2, 2) == SAO_STATUS_OK);
    }
    std::vector<SaoUiCanvasOp> snapshot(132);
    size_t written = 0;
    REQUIRE(sao_ui_script_canvas_snapshot_ops(canvas, snapshot.data(), snapshot.size(), &written) ==
            SAO_STATUS_OK);
    REQUIRE(written == snapshot.size());
    CHECK(std::memcmp(snapshot[0].aux, polygon_a, sizeof(polygon_a)) == 0);
    CHECK(std::memcmp(snapshot[1].aux, text_a.data(), text_a.size()) == 0);
    CHECK(std::memcmp(snapshot[2].aux, polygon_b, sizeof(polygon_b)) == 0);
    CHECK(std::memcmp(snapshot[3].aux, text_b.data(), text_b.size()) == 0);

    const size_t count_before = sao_ui_script_canvas_pending_op_count(canvas);
    int32_t short_polygon_aux = 7;
    std::array<SaoUiCanvasOp, 2> invalid_batch{};
    invalid_batch[0].op = SAO_UI_CANVAS_OP_LINE;
    invalid_batch[1].op = SAO_UI_CANVAS_OP_POLYGON;
    invalid_batch[1].i[0] = 2;
    invalid_batch[1].aux = &short_polygon_aux;
    invalid_batch[1].aux_len = sizeof(short_polygon_aux);
    REQUIRE(sao_ui_script_canvas_submit_ops(canvas, invalid_batch.data(), invalid_batch.size()) ==
...
TEST_CASE("canvas_bitmap_checked_budget_preserves_previous_record",
          "[ui][scriptable_canvas][runtime][hardening][bitmap][transaction]") {
    const auto canvas = make_canvas();
    const std::array<uint32_t, 4> original = {0xff0000ffU, 0xff00ff00U, 0xffff0000U, 0xffffffffU};
    int32_t bitmap_id = 0;
    REQUIRE(sao_ui_script_canvas_register_bitmap(canvas, original.data(), 2, 2, 8, &bitmap_id) ==
            SAO_STATUS_OK);

    const uint32_t dummy_pixel = 0;
    sao_status_t status = SAO_STATUS_OK;
    CHECK_NOTHROW(status = sao_ui_script_canvas_update_bitmap(
                      canvas, bitmap_id, &dummy_pixel, std::numeric_limits<uint32_t>::max(), 1, 4));
    CHECK(status == SAO_STATUS_ERR_INVALID_ARGUMENT);
    CHECK_NOTHROW(
        status = sao_ui_script_canvas_update_bitmap(canvas, bitmap_id, &dummy_pixel, 4096, 4096,
                                                    std::numeric_limits<uint32_t>::max()));
    CHECK(status == SAO_STATUS_ERR_INVALID_ARGUMENT);
    CHECK(sao_ui_script_canvas_has_bitmap(canvas, bitmap_id));
    CHECK(sao_ui_script_canvas_bitmap_count(canvas) == 1);

    int32_t rejected_id = 99;
    CHECK_NOTHROW(
        status = sao_ui_script_canvas_register_bitmap(
            canvas, &dummy_pixel, std::numeric_limits<uint32_t>::max(), 1, 4, &rejected_id));
    CHECK(status == SAO_STATUS_ERR_INVALID_ARGUMENT);
    CHECK(rejected_id == 0);
    CHECK(sao_ui_script_canvas_bitmap_count(canvas) == 1);

    REQUIRE(sao_ui_script_canvas_begin_draw(canvas) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_draw_bitmap(canvas, bitmap_id, 0, 0, 2, 2) == SAO_STATUS_OK);
    REQUIRE(sao_ui_script_canvas_end_draw(canvas) == SAO_STATUS_OK);
    SaoUiOffscreenRasterDesc desc{2, 2, 0};
    sao_ui_offscreen_raster_handle_t raster = nullptr;
    REQUIRE(sao_ui_offscreen_raster_create(&desc, &raster) == SAO_STATUS_OK);
    ...
    REQUIRE(sao_ui_offscreen_raster_snapshot(raster, pixels.data(), pixels.size(), &bytes, &width,
                                             &height, &stride) == SAO_STATUS_OK);
    REQUIRE(bytes == original.size() * sizeof(uint32_t));
    CHECK(std::memcmp(pixels.data(), original.data(), bytes) == 0);
    sao_ui_offscreen_raster_destroy(raster);

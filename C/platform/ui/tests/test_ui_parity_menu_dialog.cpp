// Menu + dialog parity contract. This file deliberately has no CMake
...
TEST_CASE("menu fixture crosses the production compositor exactly",
          "[ui][ui_parity][parity][menu][pixel]") {
    const std::vector<uint8_t> fixture = read_fixture("menu_dialog.bgra");
    ...
    backdrop_config.name_utf8 = "menu_backdrop";
    ...
    layer_config.name_utf8 = "menu_authority";
    ...
    REQUIRE(sao_ui_layer_set_alpha(layer, 1.0f) == SAO_STATUS_OK);
        ...
        alpha_config.name_utf8 = "alpha_foreground";
...
TEST_CASE("menu geometry and hit regions follow the authoritative layout",
          "[ui][ui_parity][parity][menu][geometry]") {
...
TEST_CASE("dialog authority covers geometry timings z order and keys",
          "[ui][ui_parity][parity][dialog]") {
    DialogResult result{};
...
#if defined(_WIN32)
TEST_CASE("compositor synchronizes the production host hit region",
          "[ui][ui_parity][parity][compositor][hit-region]") {
    SaoOverlayHostConfig host_config{};
    host_config.width = 64;
    host_config.height = 64;
    host_config.title_utf16 = L"menu dialog parity";
    ...
    SaoLayerConfig layer_config{};
    layer_config.name_utf8 = "hit_region";
    layer_config.x = 9;
    layer_config.y = 13;

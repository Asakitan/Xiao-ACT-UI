// Wave 7 tests for the dxgi_dup depth slice (dirty/move rects, cursor,
// staging copy).
//
...
// than fail.  Wave 3's test suite proved the create+acquire+release
// pipe on session-0 hardware, so if create succeeds and Acquire
// returns OK we know the fixture holds; the Wave 7 cases add
...
TEST_CASE("dxgi_dup_wave7_windows_only", "[ui][dxgi_dup][wave7]") {
    SUCCEED("dxgi_dup Wave 7 is Windows-only");
}

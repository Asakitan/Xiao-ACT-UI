...
- GPU Link Start uses 300 finite columns, a 32-sided/four-band shared mesh with 258 vertices and 1536 16-bit indices, stratified depth and separate foreground/fine radial populations. Current depth-tested geometry and one velocity-weighted 8.33ms exposure replace three duplicated-vertex passes; exposure is skipped at very low speed. Existing Bezier/velocity-continuous exit and zero-alpha clipping remain.
...
| Link Start | White-space five beats, indexed beveled columns, MSAA, neighborhood-clamped history, restrained UI exposure and unchanged serial PCM clock | Four shaders compiled with /WX; Debug UI linked; 20 native stills and 780-frame/60fps/13s movie fully decoded. Same 180-frame render/readback: previous single run 1.353s, current three runs 1.22..1.23s. Offline audio completed with audio=0/status=0. Independent source review passed; parent repaired non-persisting documentation updates and verified read-back. No presentation-FPS/DPI/device/acoustic acceptance claimed |
...
- Current full preview is `.sao/ui-preview/linkstart-first-generation.mp4` (SHA256 `5c79d57fb10705a058764f1b2bf523a56bbd9d7261920a69341d7df1cec541e3`); older five-stage/upgraded/reference movies are historical. Current Debug UI SHA256 is `d8fe8e252c76eacb1f499fd97d25ed32a607d826e4b76daf6c2e858f7da76a56`.

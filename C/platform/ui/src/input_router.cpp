...
constexpr uint32_t kHardBits =
    SAO_UI_MOD_CTRL_BIT  | SAO_UI_MOD_ALT_BIT |
    SAO_UI_MOD_SHIFT_BIT | SAO_UI_MOD_WIN_BIT |
    SAO_UI_MOD_CAPS_BIT  | SAO_UI_MOD_NUM_BIT;
...
bool binding_matches(uint32_t bind_mods, uint32_t observed_mods) {
    ...
    for (const auto& p : kPairs) {
        ...
        if (bind_hard && (observed_mods & p.hard) == 0) return false;
    }
...
bool hotkey_scope_matches(const sao_ui_input_router_deep_s* router,
                          const HotkeyRecord& record) {
    if (record.scope == SAO_UI_HOTKEY_SCOPE_GLOBAL) return true;
    ...
    return sao_ui_panel_find_by_id(
               router->compositor, record.scope_panel_id.c_str(), &expected) == SAO_STATUS_OK &&
...
HotkeyMatch select_hotkey_locked(const sao_ui_input_router_deep_s* router,
    ...
    for (const auto& record : router->hotkeys) {
        if (record.virtual_key != event.virtual_key) continue;
        if (event.kind == SAO_UI_INPUT_KEY_UP && !record.require_release) continue;
        if (event.kind == SAO_UI_INPUT_KEY_DOWN && record.require_release) continue;
        if (event.kind != SAO_UI_INPUT_KEY_DOWN && event.kind != SAO_UI_INPUT_KEY_UP) continue;
        if (event.key_repeat && !record.allow_repeat) continue;
        if (!binding_matches(record.modifiers, event.modifiers)) continue;
        if (!hotkey_scope_matches(router, record)) continue;
    ...
    return {best->handle, best->callback, best->user_data,
            best->binding_id, best->prevent_default};
}
...
bool region_contains(const sao_ui_input_router_deep_s::RegionCopy& region,
                     int32_t x, int32_t y) {
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_deep_create(
    ...
    sao_ui_input_router_deep_handle_t* out_handle) {

    if (out_handle == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    *out_handle = nullptr;
    auto* router = new (std::nothrow) sao_ui_input_router_deep_s();
...
extern "C" void SAO_UI_CALL sao_ui_input_router_deep_destroy(
    sao_ui_input_router_deep_handle_t handle) {
    delete handle;  // nullptr-safe
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_feed_raw_win32(
    sao_ui_input_router_deep_handle_t handle,
    uint32_t msg, uint64_t wparam, int64_t lparam,
    bool* out_consumed) {
    if (out_consumed != nullptr) *out_consumed = false;
    ...
    if (modifier_down(VK_CONTROL)) event.modifiers |= SAO_UI_MOD_CTRL_BIT;
    if (modifier_down(VK_MENU)) event.modifiers |= SAO_UI_MOD_ALT_BIT;
    if (modifier_down(VK_SHIFT)) event.modifiers |= SAO_UI_MOD_SHIFT_BIT;
    if (modifier_down(VK_LWIN) || modifier_down(VK_RWIN)) event.modifiers |= SAO_UI_MOD_WIN_BIT;
    if ((::GetKeyState(VK_CAPITAL) & 1) != 0) event.modifiers |= SAO_UI_MOD_CAPS_BIT;
    if ((::GetKeyState(VK_NUMLOCK) & 1) != 0) event.modifiers |= SAO_UI_MOD_NUM_BIT;
    ...
    case WM_XBUTTONDBLCLK:
        ...
        event.click_count = msg == WM_LBUTTONDBLCLK || msg == WM_RBUTTONDBLCLK ||
                            msg == WM_MBUTTONDBLCLK || msg == WM_XBUTTONDBLCLK ? 2 : 1;
        break;
    case WM_LBUTTONUP:
    case WM_RBUTTONUP:
    case WM_MBUTTONUP:
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_route_event(
    sao_ui_input_router_deep_handle_t handle,
    const SaoUiInputEvent* event, bool* out_consumed) {

    if (out_consumed != nullptr) *out_consumed = false;
    if (handle == nullptr || event == nullptr) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    if (!valid_event_kind(event->kind)) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::unique_lock<std::mutex> guard(handle->mu);
    ...
    sao_ui_widget_handle_t target = handle->captured_widget;
    const bool mouse_event = event->kind >= SAO_UI_INPUT_MOUSE_MOVE &&
        event->kind <= SAO_UI_INPUT_MOUSE_LEAVE;
    if (target == nullptr) {
        if (mouse_event) {
            if (event->kind != SAO_UI_INPUT_MOUSE_LEAVE) {
                if (handle->regions.empty()) {
                    if (!handle->focus_stack.empty()) target = handle->focus_stack.back().widget;
                } else {
                    for (size_t index = handle->focus_stack.size(); index-- > 0;) {
                        const auto found = handle->regions.find(handle->focus_stack[index].panel);
                        if (found != handle->regions.end() &&
                            region_contains(found->second, event->screen_x_px,
                                            event->screen_y_px)) {
                            target = handle->focus_stack[index].widget;
                            break;
                        }
                    }
                }
            }
        } else if (!handle->focus_stack.empty()) {
            target = handle->focus_stack.back().widget;
        }
    }
    ...
    if (handle->modal_top != SIZE_MAX) {
        ...
        if (target != nullptr) {
            ...
            for (size_t i = handle->modal_top;
                 i < handle->focus_stack.size(); ++i) {
                if (handle->focus_stack[i].widget == target) {
                    inside_modal = true;
                    break;
                }
            }
        ...
        if (!inside_modal) {
            ...
            if (out_consumed != nullptr) *out_consumed = true;
            return SAO_STATUS_OK;
        }
    }

    handle->last_route_target = target;
    sao_ui_hover_change_cb_t hover_callback = nullptr;
    void* hover_user_data = nullptr;
    sao_ui_widget_handle_t previous_hover = handle->hover_widget;
    sao_ui_widget_handle_t next_hover = previous_hover;
    if (event->kind == SAO_UI_INPUT_MOUSE_MOVE) next_hover = target;
    if (event->kind == SAO_UI_INPUT_MOUSE_LEAVE) next_hover = nullptr;
    if (next_hover != previous_hover) {
        handle->hover_widget = next_hover;
        hover_callback = handle->hover_cb;
        hover_user_data = handle->hover_user_data;
    }
    const HotkeyMatch hotkey = select_hotkey_locked(handle, *event);
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_set_panel_region(
    sao_ui_input_router_deep_handle_t handle,
    sao_ui_panel_handle_t panel, const SaoUiHitRegion* region) {

    if (handle == nullptr || panel == nullptr || region == nullptr) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    if (region->shape < SAO_UI_HIT_RECT || region->shape > SAO_UI_HIT_ALPHA) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    if (region->shape == SAO_UI_HIT_RECT &&
        (region->w_or_r <= 0 || region->h_or_pt_count <= 0)) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    if (region->shape == SAO_UI_HIT_CIRCLE && region->w_or_r <= 0) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    if (region->shape == SAO_UI_HIT_ALPHA &&
        (region->w_or_r <= 0 || region->h_or_pt_count <= 0)) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    ...
    if (region->shape == SAO_UI_HIT_POLYGON &&
        region->poly_verts_xy_pairs != nullptr) {
        copy.poly_verts.assign(
            region->poly_verts_xy_pairs,
            region->poly_verts_xy_pairs +
                static_cast<size_t>(region->h_or_pt_count) * 2);
    }
    handle->regions[panel] = std::move(copy);
    return SAO_STATUS_OK;
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_clear_panel_region(
    ...
    if (handle == nullptr || panel == nullptr) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    std::lock_guard<std::mutex> guard(handle->mu);
    handle->regions.erase(panel);
    return SAO_STATUS_OK;
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_set_focus_widget(
    ...
    FocusEntry entry{};
    entry.widget = widget;
    if (!handle->focus_stack.empty()) {
        entry.panel = handle->focus_stack.back().panel;
    }
    handle->focus_stack.push_back(entry);
    return SAO_STATUS_OK;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_focus_next(
    sao_ui_input_router_deep_handle_t handle, bool /*reverse*/) {

    if (handle == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    // Tab / Shift+Tab traversal needs the layout tree — routed in a
    // follow-up slice.  We accept + return OK so callers can wire the
    // key binding today.
    return SAO_STATUS_OK;
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_get_focus(
    ...
    if (out_widget != nullptr) {
        *out_widget = handle->focus_stack.empty()
                        ? nullptr
                        : handle->focus_stack.back().widget;
    }
    if (out_panel != nullptr) {
        *out_panel = handle->focus_stack.empty()
                        ? nullptr
                        : handle->focus_stack.back().panel;
    }
    return SAO_STATUS_OK;
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_push_modal(
    ...
    entry.modal = true;
    handle->focus_stack.push_back(entry);
    handle->modal_top = handle->focus_stack.size() - 1;
    return SAO_STATUS_OK;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_pop_modal(
    sao_ui_input_router_deep_handle_t handle) {

    if (handle == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> guard(handle->mu);
    // Pop entries from the top until we've removed the topmost modal.
    while (!handle->focus_stack.empty()) {
        const bool was_modal = handle->focus_stack.back().modal;
        handle->focus_stack.pop_back();
        if (was_modal) break;
    }
    // Recompute modal_top by walking backwards.
    handle->modal_top = SIZE_MAX;
    for (size_t i = handle->focus_stack.size(); i-- > 0; ) {
        if (handle->focus_stack[i].modal) {
            handle->modal_top = i;
            break;
        }
    }
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_set_hover_change_handler(
    sao_ui_input_router_deep_handle_t handle,
    sao_ui_hover_change_cb_t callback, void* user_data) {

    if (handle == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> guard(handle->mu);
    handle->hover_cb = callback;
    handle->hover_user_data = user_data;
    return SAO_STATUS_OK;
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_register_hotkey(
    ...
    const SaoUiHotkeyBindingSpec* spec,
    sao_ui_hotkey_cb_t callback, void* user_data,
    sao_ui_hotkey_binding_t* out_binding) {

    if (handle == nullptr || spec == nullptr || spec->virtual_key == 0 ||
    ...
    if (out_binding != nullptr) *out_binding = 0;
    ...
    HotkeyRecord rec{};
    rec.handle              = handle->next_hotkey_handle++;
    rec.insert_order        = handle->next_insert_order++;
    rec.plugin_id           = (plugin_id_utf8 == nullptr ? "" : plugin_id_utf8);
    rec.binding_id          = (spec->binding_id_utf8 == nullptr
                                ? ""
                                : spec->binding_id_utf8);
    rec.virtual_key         = spec->virtual_key;
    rec.modifiers           = effective_mods;
    rec.scope               = spec->scope;
    rec.scope_panel_id      = (spec->scope_panel_id_utf8 == nullptr
                                ? ""
                                : spec->scope_panel_id_utf8);
    ...
    handle->hotkeys.push_back(std::move(rec));
    if (out_binding != nullptr) {
        *out_binding = handle->hotkeys.back().handle;
    }
    return SAO_STATUS_OK;
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_unregister_hotkey(
    ...
    if (handle == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> guard(handle->mu);
    auto& v = handle->hotkeys;
    auto it = std::remove_if(v.begin(), v.end(),
        [binding](const HotkeyRecord& r) { return r.handle == binding; });
    if (it == v.end()) return SAO_STATUS_ERR_NOT_FOUND;
    v.erase(it, v.end());
    return SAO_STATUS_OK;
}

extern "C" sao_status_t SAO_UI_CALL
sao_ui_input_router_unregister_plugin_hotkeys(
    sao_ui_input_router_deep_handle_t handle,
    const char* plugin_id_utf8) {

    if (handle == nullptr || plugin_id_utf8 == nullptr) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    std::lock_guard<std::mutex> guard(handle->mu);
    ...
    v.erase(std::remove_if(v.begin(), v.end(),
        [&needle](const HotkeyRecord& r) { return r.plugin_id == needle; }),
        v.end());
    return SAO_STATUS_OK;
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_find_conflicts(
    ...
    sao_ui_hotkey_binding_t* out_bindings,
    ...
    size_t written = 0;
    for (const auto& rec : handle->hotkeys) {
        if (rec.virtual_key != virtual_key) continue;
        if (!binding_matches(rec.modifiers, modifiers)) continue;
        if (out_bindings != nullptr && written < capacity) {
            out_bindings[written] = rec.handle;
        }
        ++written;
    }
...
sao_ui_input_router_match_hotkey(
    ...
    {
        std::lock_guard<std::mutex> guard(handle->mu);
        match = select_hotkey_locked(handle, *event);
    }
    if (match.binding == 0) return SAO_STATUS_ERR_NOT_FOUND;
    if (out_binding != nullptr) *out_binding = match.binding;
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_capture_mouse(
    sao_ui_input_router_deep_handle_t handle,
    sao_ui_widget_handle_t captured_widget) {

    if (handle == nullptr || captured_widget == nullptr) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    std::lock_guard<std::mutex> guard(handle->mu);
    handle->captured_widget = captured_widget;
#if defined(_WIN32)
    void* hwnd = sao_ui_compositor_host_hwnd(handle->compositor);
    if (hwnd != nullptr) ::SetCapture(reinterpret_cast<HWND>(hwnd));
#endif
    return SAO_STATUS_OK;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_input_router_release_mouse(
    sao_ui_input_router_deep_handle_t handle) {
...
sao_ui_input_router_focus_depth(
    ...
    if (handle == nullptr) return 0;
    std::lock_guard<std::mutex> guard(handle->mu);
    return handle->focus_stack.size();
}
...
sao_ui_input_router_last_route_target(
    ...
    std::lock_guard<std::mutex> guard(handle->mu);
    return handle->last_route_target;
}
...
sao_ui_input_router_has_modal_barrier(
    ...
    std::lock_guard<std::mutex> guard(handle->mu);
    return handle->modal_top != SIZE_MAX;
}

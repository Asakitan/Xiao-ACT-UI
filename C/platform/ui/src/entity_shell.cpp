...
SAO_UI_API sao_status_t SAO_UI_CALL sao_ui_menu_compute_button_layout(
    sao_ui_menu_handle_t handle, int32_t button_index, int32_t* out_x,
...
SAO_UI_API sao_status_t SAO_UI_CALL sao_ui_menu_get_transition_progress(
    sao_ui_menu_handle_t handle, float* out_progress);
...
SAO_UI_API sao_status_t SAO_UI_CALL sao_ui_nervegear_hit_test(
    sao_ui_nervegear_handle_t handle, int32_t px, int32_t py,
    bool* out_hit);
...
constexpr int32_t kMenuWidth = 248;
constexpr int32_t kMenuHeight = 366;
constexpr int32_t kMenuSlot = 70;
constexpr int32_t kMenuTop = 8;
...
Pixel premultiply(Color color) {
    return {
        static_cast<uint8_t>((static_cast<uint32_t>(color.b) * color.a + 127U) /
                             255U),
        static_cast<uint8_t>((static_cast<uint32_t>(color.g) * color.a + 127U) /
                             255U),
        static_cast<uint8_t>((static_cast<uint32_t>(color.r) * color.a + 127U) /
                             255U),
        color.a,
    };
}
...
void blend(Raster& raster, int32_t x, int32_t y, Color color) {
    ...
    Pixel& destination =
        raster.pixels[static_cast<size_t>(y) * raster.width + x];
    const uint32_t inverse = 255U - source.a;
    destination.b = static_cast<uint8_t>(
        source.b + (static_cast<uint32_t>(destination.b) * inverse + 127U) /
                       255U);
    destination.g = static_cast<uint8_t>(
        source.g + (static_cast<uint32_t>(destination.g) * inverse + 127U) /
                       255U);
    destination.r = static_cast<uint8_t>(
        source.r + (static_cast<uint32_t>(destination.r) * inverse + 127U) /
                       255U);
    destination.a = static_cast<uint8_t>(
        source.a + (static_cast<uint32_t>(destination.a) * inverse + 127U) /
                       255U);
}

void fill_rect(Raster& raster, int32_t x, int32_t y, int32_t width,
    ...
    const int32_t bottom =
        std::min(static_cast<int32_t>(raster.height), y + height);
    for (int32_t py = top; py < bottom; ++py) {
        for (int32_t px = left; px < right; ++px) blend(raster, px, py, color);
    }
}

void fill_circle(Raster& raster, int32_t center_x, int32_t center_y,
    ...
    for (int32_t y = center_y - radius; y <= center_y + radius; ++y) {
        for (int32_t x = center_x - radius; x <= center_x + radius; ++x) {
            ...
            if (dx * dx + dy * dy <= radius_squared) blend(raster, x, y, color);
        }
    }
}

void stroke_circle(Raster& raster, int32_t center_x, int32_t center_y,
    ...
    for (int32_t y = center_y - radius; y <= center_y + radius; ++y) {
        for (int32_t x = center_x - radius; x <= center_x + radius; ++x) {
            ...
            if (distance <= outer && distance >= inner) blend(raster, x, y, color);
        }
    }
}

void draw_line(Raster& raster, int32_t x0, int32_t y0, int32_t x1,
               int32_t y1, int32_t thickness, Color color) {
    ...
    for (int32_t step = 0; step <= steps; ++step) {
        ...
        const int32_t x = static_cast<int32_t>(std::lround(
            x0 + static_cast<float>(x1 - x0) * ratio));
        const int32_t y = static_cast<int32_t>(std::lround(
            y0 + static_cast<float>(y1 - y0) * ratio));
...
void draw_text(Raster& raster, int32_t x, int32_t y,
        ...
        for (int32_t row = 0; row < static_cast<int32_t>(glyph.size()); ++row) {
            for (int32_t column = 0; column < 5; ++column) {
                if ((glyph[row] & (1U << (4 - column))) != 0U) {
                    fill_rect(raster, cursor + column * scale, y + row * scale,
                              scale, scale, color);
                }
            }
        }
...
#if defined(_WIN32)
Raster load_authority_nervegear_frame(int32_t resource_id) {
    ...
    constexpr size_t kExpectedBytes =
        static_cast<size_t>(SAO_UI_NERVEGEAR_SIZE) *
        SAO_UI_NERVEGEAR_SIZE * sizeof(Pixel);
    if (resource_bytes != kExpectedBytes) {
    ...
    Raster raster = make_raster(SAO_UI_NERVEGEAR_SIZE,
                                SAO_UI_NERVEGEAR_SIZE);
    std::memcpy(raster.pixels.data(), bytes, kExpectedBytes);
    return raster;
}
#endif
...
Raster rasterize_nervegear(SaoUiNerveGearState state) {
#if defined(_WIN32)
    const int32_t resource_id = state == SAO_UI_NG_STATE_PRESSED
        ? SAO_UI_ENTITY_AUTHORITY_NERVEGEAR_PRESSED
        : state == SAO_UI_NG_STATE_HOVER
            ? SAO_UI_ENTITY_AUTHORITY_NERVEGEAR_HOVER
            : SAO_UI_ENTITY_AUTHORITY_NERVEGEAR_IDLE;
    return load_authority_nervegear_frame(resource_id);
#else
    ...
    if (hover) stroke_circle(raster, 36, 36, 33, 2, Color{104, 228, 255, 130});
    return raster;
#endif
}
...
Raster rasterize_menu(int32_t hover_index, int32_t pressed_index) {
    ...
    fill_rect(raster, 4, 2, kMenuWidth - 8, kMenuHeight - 5,
              Color{248, 248, 248, 205});
    fill_rect(raster, 4, 2, 3, kMenuHeight - 5, Color{104, 228, 255, 235});
    fill_rect(raster, 4, kMenuHeight - 7, kMenuWidth - 8, 3,
              Color{212, 156, 23, 240});
    draw_line(raster, 10, 12, 31, 12, 2, Color{104, 228, 255, 240});
    draw_line(raster, 10, 12, 10, 31, 2, Color{104, 228, 255, 240});
    draw_line(raster, kMenuWidth - 11, kMenuHeight - 13, kMenuWidth - 32,
              kMenuHeight - 13, 2, Color{212, 156, 23, 240});
    draw_line(raster, kMenuWidth - 11, kMenuHeight - 13, kMenuWidth - 11,
              kMenuHeight - 32, 2, Color{212, 156, 23, 240});

    constexpr std::array<std::string_view, 5> labels{
        "CONTROL", "TOOLS", "PLUGINS", "SKINS", "ABOUT"};
    ...
    for (int32_t index = 0; index < static_cast<int32_t>(labels.size()); ++index) {
        ...
        const int32_t center_x = 35;
        const int32_t center_y = kMenuTop + index * kMenuSlot + 27;
        ...
        stroke_circle(raster, center_x, center_y, size / 2, 2,
                      index == kAboutIndex ? Color{212, 156, 23, 245}
                                           : Color{88, 152, 190, 225});
        draw_text(raster, center_x - 5, center_y - 7, icons[index], 2,
              Color{100, 99, 100, 255});
        draw_text(raster, 80, center_y - 7, labels[index], 2,
              pressed ? Color{98, 88, 70, 255}
                  : Color{100, 99, 100, 255});
        draw_line(raster, 73, center_y + 15, kMenuWidth - 19, center_y + 15, 1,
...
sao_status_t first_failure(sao_status_t current, sao_status_t candidate) {
    return current == SAO_STATUS_OK && candidate != SAO_STATUS_OK ? candidate
                                                                  : current;
}
...
sao_status_t set_menu_visibility_locked(sao_ui_entity_shell_s* shell,
                                        bool visible) {
    const bool changed = shell->menu_visible != visible;
    const sao_status_t status = visible
        ? sao_ui_menu_show(shell->menu, shell->menu_x + 35,
                           shell->menu_y + kMenuTop)
...
sao_status_t refresh_host_geometry_locked(sao_ui_entity_shell_s* shell) {
    if (shell->host == nullptr) return SAO_STATUS_OK;
    ...
    if (geometry.width <= 0 || geometry.height <= 0) return SAO_STATUS_OK;
    ...
    shell->nervegear_x = std::max(
        0, shell->width - kMarginRight - SAO_UI_NERVEGEAR_SIZE);
    shell->nervegear_y = std::max(
        0, shell->height - kMarginBottom - SAO_UI_NERVEGEAR_SIZE);
    const int32_t menu_max_x = std::max(0, shell->width - kMenuWidth);
    const int32_t menu_max_y = std::max(0, shell->height - kMenuHeight);
    shell->menu_x = std::clamp(
        shell->nervegear_x + SAO_UI_NERVEGEAR_SIZE - kMenuWidth,
        0, menu_max_x);
    shell->menu_y = std::clamp(
        shell->nervegear_y - kMenuHeight - 12, 0, menu_max_y);

    SaoUiMenuLayout layout{};
    layout.center_x = shell->menu_x + 35;
    layout.center_y = shell->menu_y + kMenuTop;
    ...
    status = first_failure(
        status, sao_ui_nervegear_set_position(
                    shell->nervegear,
                    shell->origin_x + shell->nervegear_x,
                    shell->origin_y + shell->nervegear_y));
    status = first_failure(
        status, sao_ui_layer_set_position(
                    shell->nervegear_layer, shell->nervegear_x,
                    shell->nervegear_y));
    status = first_failure(
        status, sao_ui_layer_set_position(shell->menu_layer, shell->menu_x,
                                          shell->menu_y));
    if (status == SAO_STATUS_OK) shell->visual_dirty = true;
    return status;
}

sao_status_t apply_layer_state_locked(sao_ui_entity_shell_s* shell) {
    const bool nervegear_active =
        shell->online && shell->overlay_visible;
    ...
    status = first_failure(
        status, sao_ui_menu_get_transition_progress(
                    shell->menu, &menu_progress));
    const bool menu_active =
        nervegear_active && menu_phase != SAO_UI_MENU_PHASE_CLOSED;
    const float menu_layer_alpha =
        shell->menu_visible && menu_phase == SAO_UI_MENU_PHASE_OPENING
        ? std::max(menu_progress, 1.0F / 255.0F)
        : menu_progress;
    status = first_failure(
        status, sao_ui_layer_set_visible(
                    shell->nervegear_layer, nervegear_active));
    status = first_failure(
        status, sao_ui_layer_set_alpha(shell->menu_layer, menu_layer_alpha));
    status = first_failure(
        status, sao_ui_layer_set_input_enabled(
                    shell->nervegear_layer, nervegear_active));
    status = first_failure(
        status, sao_ui_layer_set_visible(shell->menu_layer, menu_active));
    status = first_failure(
        status,
        sao_ui_layer_set_input_enabled(
...
sao_status_t raster_and_upload_locked(sao_ui_entity_shell_s* shell) {
    ...
    try {
        ...
        shell->menu_raster = rasterize_menu(shell->menu_hover_index,
                                            shell->menu_pressed_index);
    } catch (...) {
    ...
    return sao_ui_layer_update_bgra(
        shell->menu_layer,
        reinterpret_cast<const uint8_t*>(shell->menu_raster.pixels.data()),
...
sao_status_t commit_visual_state_locked(sao_ui_entity_shell_s* shell) {
    if (!shell->visual_dirty) {
        ...
        const sao_status_t settle_status =
            sao_ui_compositor_sync_host_rgn(shell->compositor);
    ...
    if (shell->host != nullptr) {
        status = first_failure(status,
                               sao_ui_compositor_present(shell->compositor));
        const sao_status_t region_status =
            sao_ui_compositor_sync_host_rgn(shell->compositor);
        ...
        status = first_failure(
            status, sao_ui_compositor_sync_host_input_mode(shell->compositor));
        status = first_failure(
            status, sao_ui_compositor_enforce_z_order(shell->compositor));
    }
    ++shell->frame_count;
    if (status == SAO_STATUS_OK) shell->visual_dirty = false;
    shell->last_status = status;
    return status;
}

sao_status_t sync_frame_locked(sao_ui_entity_shell_s* shell,
                               uint32_t elapsed_ms) {
    ...
    status = first_failure(
        status, sao_ui_nervegear_get_state(shell->nervegear, &previous_state));
    status = first_failure(
        status, sao_ui_menu_get_phase(shell->menu, &previous_menu_phase));
    status = first_failure(
        status, sao_ui_menu_get_transition_progress(
                    shell->menu, &previous_menu_progress));
    status = first_failure(
        status, sao_ui_nervegear_tick(
                    shell->nervegear, static_cast<int32_t>(elapsed_ms)));
    status = first_failure(
        status,
        sao_ui_menu_tick(shell->menu, static_cast<int32_t>(elapsed_ms)));
    ...
    status = first_failure(
        status, sao_ui_nervegear_get_state(shell->nervegear, &current_state));
    status = first_failure(
        status, sao_ui_menu_get_phase(shell->menu, &current_menu_phase));
    status = first_failure(
        status, sao_ui_menu_get_transition_progress(
                    shell->menu, &current_menu_progress));
    shell->visual_dirty = shell->visual_dirty ||
...
bool local_nervegear_hit_locked(sao_ui_entity_shell_s* shell,
                                int32_t screen_x, int32_t screen_y) {
    bool hit = false;
    return sao_ui_nervegear_hit_test(shell->nervegear, screen_x, screen_y,
                                     &hit) == SAO_STATUS_OK &&
           hit;
}

int32_t menu_hit_locked(sao_ui_entity_shell_s* shell, int32_t screen_x,
                        int32_t screen_y) {
    int32_t index = -1;
    int32_t child = -1;
    if (!shell->menu_visible ||
        sao_ui_menu_hit_test(shell->menu, screen_x - shell->origin_x,
                             screen_y - shell->origin_y, &index,
                             &child) != SAO_STATUS_OK) {
        return -1;
    }
    return index;
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_entity_shell_create(
    sao_ui_overlay_host_handle_t host, const SaoUiEntityShellConfig* config,
    sao_ui_entity_shell_handle_t* out_handle) {
    if (out_handle == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    *out_handle = nullptr;
    SaoUiEntityShellConfig effective = config == nullptr
        ? SaoUiEntityShellConfig{}
        : *config;
    SaoOverlayHostState host_state{};
    if (host != nullptr) {
        const sao_status_t host_status =
            sao_ui_overlay_host_get_state(host, &host_state);
        if (host_status != SAO_STATUS_OK) return host_status;
    ...
    auto shell = std::unique_ptr<sao_ui_entity_shell_s>(
        new (std::nothrow) sao_ui_entity_shell_s());
    if (shell == nullptr) return SAO_STATUS_ERR_UNKNOWN;
    shell->host = host;
    ...
    shell->nervegear_x = std::max(
        0, effective.width - kMarginRight - SAO_UI_NERVEGEAR_SIZE);
    shell->nervegear_y = std::max(
        0, effective.height - kMarginBottom - SAO_UI_NERVEGEAR_SIZE);
    ...
    shell->menu_x = std::clamp(shell->nervegear_x + SAO_UI_NERVEGEAR_SIZE -
                                   kMenuWidth,
                               0, menu_max_x);
    shell->menu_y = std::clamp(shell->nervegear_y - kMenuHeight - 12, 0,
                               menu_max_y);
    shell->owner_thread = std::this_thread::get_id();

    sao_status_t status = sao_ui_compositor_create(
        host, nullptr, &shell->compositor);
    ...
    if (status == SAO_STATUS_OK) {
        status = sao_ui_menu_create(shell->compositor, shell->theme,
                                    SAO_UI_MENU_MODE_VERTICAL_STRIP,
                                    &shell->menu);
    }
    if (status == SAO_STATUS_OK) {
        const std::array<SaoUiMenuItem, 5> items{{
            ...
            {"About", "?", SAO_UI_ENTITY_ACTION_OPEN_ABOUT, true,
             {false, false, false}},
        }};
    ...
    if (status == SAO_STATUS_OK) {
        status = sao_ui_nervegear_create(
            shell->compositor, shell->theme,
            shell->origin_x + shell->nervegear_x,
            shell->origin_y + shell->nervegear_y, SAO_UI_NG_PALETTE_DARK,
            &shell->nervegear);
    }
    if (status == SAO_STATUS_OK) {
        ...
        layer.high_fps = true;
        layer.target_fps = 60;
        status = sao_ui_layer_create(shell->compositor, &layer,
                                     &shell->nervegear_layer);
    }
    if (status == SAO_STATUS_OK) {
        SaoLayerConfig layer{};
    ...
    if (status == SAO_STATUS_OK) {
        ...
        for (int32_t index = 0;
             index < static_cast<int32_t>(input_rects.size()); ++index) {
            int32_t x = 0;
            int32_t y = 0;
            int32_t width = 0;
            int32_t height = 0;
            status = sao_ui_menu_compute_button_layout(
                shell->menu, index, &x, &y, &width, &height);
            if (status != SAO_STATUS_OK) break;
            input_rects[static_cast<size_t>(index)] = {
                x - shell->menu_x, y - shell->menu_y, width, height};
        }
        if (status == SAO_STATUS_OK) {
            status = sao_ui_layer_set_input_rects(
                shell->menu_layer, input_rects.data(), input_rects.size());
        }
    }
    if (status == SAO_STATUS_OK) {
        status = sao_ui_layer_set_input_enabled(shell->menu_layer, false);
    }
    if (status == SAO_STATUS_OK) status = raster_and_upload_locked(shell.get());
    if (status != SAO_STATUS_OK) {
...
extern "C" void SAO_UI_CALL sao_ui_entity_shell_destroy(
    ...
    if (std::this_thread::get_id() != handle->owner_thread) return;
    (void)sao_ui_entity_shell_take_offline(handle);
    destroy_members(handle);
    delete handle;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_entity_shell_bring_online(
    sao_ui_entity_shell_handle_t handle) {
    ...
    if (handle->online) return SAO_STATUS_OK;
    handle->online = true;
    handle->overlay_visible = true;
    handle->visual_dirty = true;
    sao_status_t status = sao_ui_nervegear_show(handle->nervegear);
    status = first_failure(
        status, sao_ui_layer_set_visible(handle->nervegear_layer, true));
    status = first_failure(
        status, sao_ui_layer_set_input_enabled(handle->nervegear_layer, true));
    status = first_failure(status, sync_frame_locked(handle, 0));
    if (handle->host != nullptr) {
        status = first_failure(
            status, sao_ui_overlay_host_set_visible(handle->host, true));
    }
    if (status != SAO_STATUS_OK) {
        if (handle->host != nullptr) {
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_entity_shell_take_offline(
    sao_ui_entity_shell_handle_t handle) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (std::this_thread::get_id() != handle->owner_thread) {
        return SAO_STATUS_ERR_ACCESS_DENIED;
    ...
    if (handle->host != nullptr) {
        status = first_failure(
            status, sao_ui_overlay_host_set_visible(handle->host, false));
    }
    handle->last_status = status;
    return status;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_entity_shell_tick(
    sao_ui_entity_shell_handle_t handle, uint32_t elapsed_ms) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (elapsed_ms > 1000U) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    if (std::this_thread::get_id() != handle->owner_thread) {
        return SAO_STATUS_ERR_ACCESS_DENIED;
    }
    std::lock_guard<std::mutex> lock(handle->mutex);
    if (!handle->online) return SAO_STATUS_ERR_NOT_INITIALIZED;
    return sync_frame_locked(handle, elapsed_ms);
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_entity_shell_handle_mouse(
    sao_ui_entity_shell_handle_t handle, uint32_t message, int32_t screen_x,
    int32_t screen_y, int32_t button, int32_t wheel_delta) {
    (void)wheel_delta;
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (std::this_thread::get_id() != handle->owner_thread) {
        return SAO_STATUS_ERR_ACCESS_DENIED;
    ...
    sao_status_t status = SAO_STATUS_OK;
    {
        ...
        const bool nervegear_hit = local_nervegear_hit_locked(
            handle, screen_x, screen_y);
        const int32_t menu_index = menu_hit_locked(handle, screen_x, screen_y);
        SaoUiNerveGearState previous_state = SAO_UI_NG_STATE_IDLE;
        status = sao_ui_nervegear_get_state(handle->nervegear,
                                            &previous_state);
        ...
        if (message == kMouseMove) {
            ...
            if (nervegear_hit) {
                ...
                status = first_failure(
                    status, sao_ui_nervegear_transition(
                                handle->nervegear, SAO_UI_NG_STATE_IDLE));
                status = first_failure(
                    status,
                    sao_ui_nervegear_on_mouse_enter(handle->nervegear));
            } else if (menu_index >= 0 &&
                       menu_index == handle->menu_pressed_index) {
        ...
        status = first_failure(
            status, sao_ui_nervegear_get_state(handle->nervegear,
                                                &current_state));
        handle->visual_dirty = handle->visual_dirty ||
            previous_state != current_state ||
    ...
    if (status == SAO_STATUS_OK && action_fn != nullptr) {
        ...
        try {
            action_status = action_fn(SAO_UI_ENTITY_ACTION_OPEN_ABOUT,
                                      action_user_data);
        } catch (...) {
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_entity_shell_hit_test(
    sao_ui_entity_shell_handle_t handle, int32_t screen_x, int32_t screen_y,
    bool* out_hit) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (out_hit == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->mutex);
    *out_hit = false;
    if (!handle->online || !handle->overlay_visible) return SAO_STATUS_OK;
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_entity_shell_home(
    sao_ui_entity_shell_handle_t handle) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (std::this_thread::get_id() != handle->owner_thread) {
        return SAO_STATUS_ERR_ACCESS_DENIED;
    ...
    if (!handle->online) return SAO_STATUS_ERR_NOT_INITIALIZED;
    sao_status_t status =
        set_menu_visibility_locked(handle, !handle->menu_visible);
    status = first_failure(status, commit_visual_state_locked(handle));
    handle->last_status = status;
    return status;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_entity_shell_insert(
    sao_ui_entity_shell_handle_t handle) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (std::this_thread::get_id() != handle->owner_thread) {
        return SAO_STATUS_ERR_ACCESS_DENIED;
    ...
    if (handle->host != nullptr) {
        status = first_failure(
            status, sao_ui_overlay_host_set_visible(
                        handle->host, handle->overlay_visible));
    }
    handle->last_status = status;
    return status;
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_entity_shell_get_snapshot(
    ...
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (out_snapshot == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->mutex);
    *out_snapshot = {};
    ...
    (void)sao_ui_nervegear_get_state(handle->nervegear,
                                     &out_snapshot->nervegear_state);
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_entity_shell_snapshot_bgra(
    sao_ui_entity_shell_handle_t handle, uint8_t* out_bgra_pixels,
    size_t capacity, uint32_t* out_width, uint32_t* out_height,
    size_t* out_bytes) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    std::lock_guard<std::mutex> lock(handle->mutex);
    return sao_ui_compositor_snapshot_bgra(
        handle->compositor, out_bgra_pixels, capacity, out_width, out_height,
        out_bytes);
}

...
static_assert(sizeof(sao_ui_menu_handle_t) == sizeof(void*),
              "menu handle must be pointer-width");
static_assert(SAO_UI_MENU_MODE_VERTICAL_STRIP == 0,
              "menu mode enum drifted from header");
static_assert(SAO_UI_MENU_MODE_RING == 1,
              "menu mode enum drifted from header");
static_assert(SAO_UI_MENU_MODE_CASCADE == 2,
              "menu mode enum drifted from header");
static_assert(SAO_UI_MENU_PHASE_CLOSED == 0,
              "menu phase enum drifted from header");
static_assert(SAO_UI_MENU_PHASE_CLOSING == 6,
              "menu phase enum drifted from header");
static_assert(SAO_UI_MENU_BTN_IDLE == 0,
              "menu button state enum drifted from header");
static_assert(SAO_UI_MENU_BTN_DISABLED == 3,
              "menu button state enum drifted from header");
...
void merge_layout_overrides(SaoUiMenuLayout* dst, const SaoUiMenuLayout* src) {
    ...
    if (src->center_x          != 0) dst->center_x          = src->center_x;
    if (src->center_y          != 0) dst->center_y          = src->center_y;
    if (src->inner_radius      != 0) dst->inner_radius      = src->inner_radius;
    if (src->outer_radius      != 0) dst->outer_radius      = src->outer_radius;
    if (src->child_ring_radius != 0) dst->child_ring_radius = src->child_ring_radius;
    if (src->button_size       != 0) dst->button_size       = src->button_size;
    if (src->button_max_size   != 0) dst->button_max_size   = src->button_max_size;
    if (src->slot_size         != 0) dst->slot_size         = src->slot_size;
    if (src->max_visible       != 0) dst->max_visible       = src->max_visible;
}
...
SaoUiMenuButtonRect compute_ring_rect(const SaoUiMenuLayout& layout,
                                      int32_t button_index,
                                      int32_t button_count) {
    const int32_t size = layout.button_size > 0 ? layout.button_size
                                                : kDefaultButtonSize;
    if (button_count <= 0) {
        return SaoUiMenuButtonRect{layout.center_x - size / 2,
                                   layout.center_y - size / 2,
                                   size, size};
    }
...
SaoUiMenuButtonRect compute_vertical_rect(const SaoUiMenuLayout& layout,
                                          int32_t button_index) {
    const int32_t size = layout.button_size > 0 ? layout.button_size
                                                : kDefaultButtonSize;
    const int32_t slot = layout.slot_size > 0 ? layout.slot_size
                                              : kDefaultSlotSize;
    // The SE anchor lives at (center_x, center_y); the strip grows
    // downward from there.  Buttons are horizontally centred on the
    // anchor column.
    const int32_t x = layout.center_x - size / 2;
...
SaoUiMenuButtonRect compute_cascade_rect(const SaoUiMenuLayout& layout,
                                         int32_t button_index,
                                         int32_t button_count) {
    const int32_t size = layout.button_size > 0 ? layout.button_size
                                                : kDefaultButtonSize;
...
PendingMenuEvent capture_event_locked(sao_ui_menu_s* menu,
                                      SaoUiMenuEvent event,
                                      int32_t primary,
                                      int32_t secondary,
                                      int32_t action_id) {
    return {menu->callback, menu->callback_user_data, event, primary,
            secondary, action_id};
}

void dispatch_event_noexcept(const PendingMenuEvent& pending) noexcept {
    if (pending.callback == nullptr) return;
    try {
        pending.callback(pending.event, pending.primary, pending.secondary,
                         pending.action_id, pending.user_data);
    } catch (...) {
    }
}
...
int32_t ring_hit_test(const SaoUiMenuLayout& layout,
                      int32_t button_count,
    ...
    if (idx < 0) idx = 0;
    if (idx >= button_count) idx = 0;    // wrap seam: N maps back to 0
    return idx;
}

int32_t vertical_hit_test(const SaoUiMenuLayout& layout,
                          int32_t button_count,
                          int32_t px, int32_t py) {
...
int32_t cascade_hit_test(const SaoUiMenuLayout& layout,
                         int32_t button_count,
                         int32_t px, int32_t py) {
    for (int32_t i = 0; i < button_count; ++i) {
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_create(
    ...
    *out_handle = nullptr;
    if (mode != SAO_UI_MENU_MODE_VERTICAL_STRIP &&
        mode != SAO_UI_MENU_MODE_RING &&
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_set_items(
    sao_ui_menu_handle_t handle,
    const SaoUiMenuItem* items,
    size_t item_count) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (items == nullptr && item_count > 0) return SAO_STATUS_ERR_INVALID_ARGUMENT;
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_set_children(
    sao_ui_menu_handle_t handle,
    const char* parent_name_utf8,
    const SaoUiMenuItem* items,
    size_t item_count) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (parent_name_utf8 == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    if (items == nullptr && item_count > 0) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->mtx);
    // Reject if parent not registered.
    bool found_parent = false;
    for (const auto& it : handle->items) {
        if (it.name == parent_name_utf8) { found_parent = true; break; }
    }
    if (!found_parent) return SAO_STATUS_ERR_NOT_FOUND;
    // Replace-or-insert.
    ChildMenu* target = nullptr;
    for (auto& cm : handle->children) {
        if (cm.parent_name == parent_name_utf8) { target = &cm; break; }
    }
    if (target == nullptr) {
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_set_layout(
    sao_ui_menu_handle_t handle,
    const SaoUiMenuLayout* layout) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (layout == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->mtx);
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_show(
    sao_ui_menu_handle_t handle,
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_hide(
    sao_ui_menu_handle_t handle) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    std::lock_guard<std::mutex> lock(handle->mtx);
    if (handle->phase == SAO_UI_MENU_PHASE_CLOSED) {
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_is_visible(
    sao_ui_menu_handle_t handle, bool* out_visible) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (out_visible == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->mtx);
    *out_visible = (handle->phase != SAO_UI_MENU_PHASE_CLOSED);
    return SAO_STATUS_OK;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_get_phase(
    sao_ui_menu_handle_t handle, SaoUiMenuPhase* out_phase) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (out_phase == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->mtx);
    *out_phase = handle->phase;
    return SAO_STATUS_OK;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_hit_test(
    sao_ui_menu_handle_t handle,
    int32_t x, int32_t y,
    int32_t* out_menu_idx,
    int32_t* out_child_idx) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (out_menu_idx == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->mtx);
    *out_menu_idx = -1;
    if (out_child_idx) *out_child_idx = -1;
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_set_hover(
    sao_ui_menu_handle_t handle, int32_t menu_idx) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    PendingMenuEvent pending{};
    {
        ...
        if (handle->hover_idx == menu_idx) return SAO_STATUS_OK;
        ...
        pending = capture_event_locked(
            handle, SAO_UI_MENU_EV_HOVER_CHANGED, menu_idx, -1, 0);
    }
    dispatch_event_noexcept(pending);
    return SAO_STATUS_OK;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_activate(
    sao_ui_menu_handle_t handle, int32_t menu_idx) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    PendingMenuEvent pending{};
    {
        ...
        auto& it = handle->items[menu_idx];
        if (!it.can_activate) return SAO_STATUS_OK;
        handle->active_idx = menu_idx;
        it.state = SAO_UI_MENU_BTN_ACTIVE;
        pending = capture_event_locked(
            handle, SAO_UI_MENU_EV_ITEM_ACTIVATED, menu_idx, -1,
            it.action_id);
    }
    dispatch_event_noexcept(pending);
    return SAO_STATUS_OK;
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_menu_query_hud_bounds(
    sao_ui_menu_handle_t handle,
    SaoUiMenuHudBounds* out_bounds) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (out_bounds == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->mtx);
...
sao_ui_menu_compute_button_layout(
    sao_ui_menu_handle_t handle,
    int32_t button_index,
...
extern "C" SAO_UI_API sao_status_t SAO_UI_CALL
sao_ui_menu_tick(sao_ui_menu_handle_t handle, int32_t dt_ms) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
        ...
        switch (handle->phase) {
            case SAO_UI_MENU_PHASE_OPENING:
                if (handle->phase_elapsed_ms >= kMenuOpenMs) {
                    ...
                    handle->phase_elapsed_ms = 0;
                    pending = capture_event_locked(
                        handle, SAO_UI_MENU_EV_OPENED, -1, -1, 0);
                }
                break;
            case SAO_UI_MENU_PHASE_CLOSING:
                if (handle->phase_elapsed_ms >= kMenuCloseMs) {
                    ...
                    handle->phase_elapsed_ms = 0;
                    pending = capture_event_locked(
                        handle, SAO_UI_MENU_EV_CLOSED, -1, -1, 0);
                }
                break;
...
extern "C" SAO_UI_API sao_status_t SAO_UI_CALL
sao_ui_menu_get_transition_progress(sao_ui_menu_handle_t handle,
                                    float* out_progress) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (out_progress == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->mtx);
    switch (handle->phase) {
...
extern "C" SAO_UI_API sao_status_t SAO_UI_CALL
sao_ui_menu_set_button_state(sao_ui_menu_handle_t handle,
                             int32_t button_index,
    ...
    if (state == SAO_UI_MENU_BTN_HOVER) handle->hover_idx = button_index;
    if (state == SAO_UI_MENU_BTN_ACTIVE) handle->active_idx = button_index;
    return SAO_STATUS_OK;
}

// Query a button's current visual state.
extern "C" SAO_UI_API sao_status_t SAO_UI_CALL
sao_ui_menu_get_button_state(sao_ui_menu_handle_t handle,
                             int32_t button_index,
...
extern "C" SAO_UI_API sao_status_t SAO_UI_CALL
sao_ui_menu_dispatch_event(sao_ui_menu_handle_t handle,
                           SaoUiMenuEvent event_type,
                           const void* data_ptr) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    switch (event_type) {
        case SAO_UI_MENU_EV_ITEM_ACTIVATED: {
            if (data_ptr == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
        ...
        case SAO_UI_MENU_EV_BACKGROUND_CLICK: {
            PendingMenuEvent pending{};
            {
                std::lock_guard<std::mutex> lock(handle->mtx);
                pending = capture_event_locked(
                    handle, SAO_UI_MENU_EV_BACKGROUND_CLICK, -1, -1, 0);
            }
            dispatch_event_noexcept(pending);

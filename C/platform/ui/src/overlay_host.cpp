...
bool acquire_single_instance_lock() {
    ...
    if (global_instance_handle() != nullptr) return false;
#if defined(SAO_UI_OVERLAY_HOST_TESTING)
...
sao_status_t apply_passthrough_unlocked(sao_ui_overlay_host_s* host, bool passthrough) {
    ...
    if (new_style != old_style) {
        ...
        if (!api.set_window_pos(host->hwnd, nullptr, 0, 0, 0, 0,
                                SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE |
                                    SWP_FRAMECHANGED)) {
            if (!restore_passthrough_style(host, old_style)) mark_input_partial(host);
...
bool append_rect(HRGN destination, const SaoOverlayHostInputRect& rect) {
    ...
    if (rect.width <= 0 || rect.height <= 0 || right > INT32_MAX || bottom > INT32_MAX) return false;
...
void dispatch_mouse(sao_ui_overlay_host_s* host, UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    if (message != WM_MOUSEWHEEL) ::ClientToScreen(host->hwnd, &point);
    int32_t button = -1;
...
LRESULT CALLBACK overlay_wndproc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    auto* host = reinterpret_cast<sao_ui_overlay_host_s*>(
    ...
    if (message == WM_NCHITTEST) {
        ...
        if (passthrough) return HTTRANSPARENT;
        return callback == nullptr || callback(GET_X_LPARAM(lparam), GET_Y_LPARAM(lparam), user_data)
                   ? HTCLIENT
                   : HTTRANSPARENT;
    }
    ...
    case WM_SIZE: {
        ...
        if (callback != nullptr) callback(width, height, user_data);
        return 0;
    }
    case WM_MOVE: {
        ...
        if (callback != nullptr) callback(x, y, user_data);
        return 0;
    }
    case WM_ACTIVATE: {
        ...
        if (callback != nullptr) callback(LOWORD(wparam) != WA_INACTIVE, user_data);
        return 0;
    }
    ...
    case WM_DPICHANGED: {
        ...
        if (suggested != nullptr) {
            ...
            ::SetWindowPos(hwnd, nullptr, x, y, width, height, SWP_NOACTIVATE | SWP_NOZORDER);
        }
        ...
        void* user_data = nullptr;
        {
            ...
            if (suggested != nullptr) host->client_rect = {x, y, width, height};
            callback = host->dpi_changed_fn;
            user_data = host->dpi_changed_user;
        }
        if (callback != nullptr) callback(dpi, x, y, width, height, user_data);
        return 0;
    }
...
void destroy_created_host(sao_ui_overlay_host_s* host) {
    if (host->hwnd != nullptr) ::DestroyWindow(host->hwnd);
    if (host->control_hwnd != nullptr) ::DestroyWindow(host->control_hwnd);
    if (host->owner_hwnd != nullptr) ::DestroyWindow(host->owner_hwnd);
    if (host->class_atom != 0) ::UnregisterClassW(host->class_name.c_str(), host->hinstance);
    delete host;
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_create(
    ...
    *out_handle = nullptr;
    if (!acquire_single_instance_lock()) return SAO_STATUS_ERR_ALREADY_EXISTS;
    ...
    const int32_t height = explicit_bounds ? config->height : ::GetSystemMetrics(SM_CYVIRTUALSCREEN);
    ...
    const wchar_t* title = config != nullptr && config->title_utf16 != nullptr ? config->title_utf16
                                                                                : L"SAO Overlay Host";

    host->owner_hwnd = ::CreateWindowExW(WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE,
                                         host->class_name.c_str(), L"", WS_POPUP, 0, 0, 0, 0, nullptr,
                                         nullptr, host->hinstance, host);
    host->control_hwnd = host->owner_hwnd == nullptr
                             ? nullptr
                             : ::CreateWindowExW(WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE,
                                                 host->class_name.c_str(), L"", WS_POPUP, 0, 0, 1, 1,
    ...
    host->dc_mutation = config == nullptr
        ...
        : static_cast<sao_ui_dc_mutation_coordinator_handle_t>(
              config->dc_mutation_coordinator);
...
extern "C" bool SAO_UI_CALL sao_ui_overlay_host_destroy(sao_ui_overlay_host_handle_t handle) {
    ...
    if (handle->owner_thread_id != ::GetCurrentThreadId()) return false;
    if (handle->dc_mutation != nullptr && handle->hwnd != nullptr &&
        !sao_ui_dc_mutation_coordinator_invalidate(
            handle->dc_mutation, handle->hwnd, 1.0)) {
        return false;
    }
...
#endif
    if (handle->hwnd != nullptr && !::DestroyWindow(handle->hwnd)) return false;
    handle->hwnd = nullptr;
    if (handle->control_hwnd != nullptr && !::DestroyWindow(handle->control_hwnd)) return false;
    handle->control_hwnd = nullptr;
    if (handle->owner_hwnd != nullptr && !::DestroyWindow(handle->owner_hwnd)) return false;
    handle->owner_hwnd = nullptr;
    if (handle->class_atom != 0) ::UnregisterClassW(handle->class_name.c_str(), handle->hinstance);
    release_single_instance_lock();
    delete handle;
    return true;
}
...
extern "C" void* SAO_UI_CALL sao_ui_overlay_host_dc_mutation_coordinator(
    sao_ui_overlay_host_handle_t handle) {
    return handle == nullptr ? nullptr : handle->dc_mutation;
}

extern "C" void* SAO_UI_CALL sao_ui_overlay_host_hglrc(sao_ui_overlay_host_handle_t) { return nullptr; }
extern "C" void* SAO_UI_CALL sao_ui_overlay_host_hdc(sao_ui_overlay_host_handle_t) { return nullptr; }
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_set_bounds(
    ...
    if (handle->dc_mutation != nullptr) {
        const std::string args = "{\"x\":" + std::to_string(x) +
            ",\"y\":" + std::to_string(y) +
            ",\"width\":" + std::to_string(width) +
            ",\"height\":" + std::to_string(height) + "}";
        const sao_status_t submit_status = sao_ui_dc_mutation_coordinator_submit_dc(
            handle->dc_mutation, handle->hwnd, "host-rect", "set_window_rect",
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_set_visible(
    sao_ui_overlay_host_handle_t handle, bool visible) {
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_set_input_passthrough(
    sao_ui_overlay_host_handle_t handle, bool passthrough) {
...
extern "C" bool SAO_UI_CALL sao_ui_overlay_host_input_passthrough(
    sao_ui_overlay_host_handle_t handle) {
    if (handle == nullptr) return false;
    std::lock_guard<std::mutex> lock(handle->state_mu);
...
extern "C" uint32_t SAO_UI_CALL sao_ui_overlay_host_input_sync_state(
    sao_ui_overlay_host_handle_t handle) {
    if (handle == nullptr) return SAO_UI_OVERLAY_INPUT_PARTIAL;
    std::lock_guard<std::mutex> lock(handle->state_mu);
    return handle->input_sync_state;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_set_capture_mode(
    sao_ui_overlay_host_handle_t handle, bool exclude) {
    if (handle == nullptr || handle->hwnd == nullptr || handle->control_hwnd == nullptr) {
        return SAO_STATUS_ERR_HANDLE_INVALID;
    }
#if defined(SAO_UI_HAS_ANTI_SCREENCAP_FACADE)
    ...
    const int32_t rc = sao_security_anti_screencap_overlay_host_set_capture_mode(
        &pair, exclude ? SAO_ASC_MODE_EXCLUDED : SAO_ASC_MODE_NORMAL,
        &effective);
    if (rc != SAO_STATUS_OK) {
        return rc;
    }
    ...
    handle->capture_excluded = effective == SAO_ASC_MODE_STREAMING
                            || effective == SAO_ASC_MODE_EXCLUDED;
    return SAO_STATUS_OK;
#else
    std::lock_guard<std::mutex> lock(handle->state_mu);
    handle->capture_excluded = false;
...
extern "C" bool SAO_UI_CALL sao_ui_overlay_host_capture_excluded(
    sao_ui_overlay_host_handle_t handle) {
    if (handle == nullptr) return false;
    std::lock_guard<std::mutex> lock(handle->state_mu);
...
extern "C" bool SAO_UI_CALL sao_ui_overlay_host_visible(
    sao_ui_overlay_host_handle_t handle) {
    if (handle == nullptr) return false;
    std::lock_guard<std::mutex> lock(handle->state_mu);
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_get_state(
    sao_ui_overlay_host_handle_t handle, SaoOverlayHostState* out_state) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_make_current(
    sao_ui_overlay_host_handle_t) {
    return SAO_STATUS_ERR_NOT_IMPLEMENTED;  // legacy WGL stub
}
extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_release_current(
    sao_ui_overlay_host_handle_t) {
    return SAO_STATUS_ERR_NOT_IMPLEMENTED;  // legacy WGL stub
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_pump_messages(
    sao_ui_overlay_host_handle_t handle) {
    if (handle == nullptr || handle->hwnd == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    MSG message{};
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_msg_wait(
    sao_ui_overlay_host_handle_t handle, uint32_t timeout_ms) {
    if (handle == nullptr || handle->hwnd == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    return ::MsgWaitForMultipleObjectsEx(0, nullptr, timeout_ms, QS_ALLINPUT, MWMO_INPUTAVAILABLE) ==
                   WAIT_FAILED
               ? SAO_STATUS_ERR_OS_CALL_FAILED
               : SAO_STATUS_OK;
}
...
extern "C" uint32_t SAO_UI_CALL sao_ui_overlay_host_current_dpi(
    sao_ui_overlay_host_handle_t handle) {
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_overlay_host_get_wm_counters(
    ...
    if (out_counters != nullptr) std::memset(out_counters, 0, sizeof(*out_counters));
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (out_counters == nullptr) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->state_mu);

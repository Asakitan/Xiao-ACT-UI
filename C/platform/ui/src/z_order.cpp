...
extern "C" void SAO_UI_CALL sao_ui_z_order_manager_destroy(sao_ui_z_order_manager_handle_t handle) {
    delete handle;
}

extern "C" sao_status_t SAO_UI_CALL
sao_ui_z_order_set_policy(sao_ui_z_order_manager_handle_t handle, int32_t policy) {
    if (handle == nullptr)
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_z_order_enforce(sao_ui_z_order_manager_handle_t handle,
                                                           void* game_hwnd, bool game_is_topmost,
                                                           bool game_present) {
    if (handle == nullptr)
        return SAO_STATUS_ERR_HANDLE_INVALID;
    std::lock_guard<std::mutex> lock(handle->mu);
    ...
    if (bounds_status != SAO_STATUS_OK)
    ...
    const uint64_t interval_ns =
        static_cast<uint64_t>(game_present ? handle->active_ms : handle->idle_ms) * 1000000ULL;
    ...
    if (handle->policy == SAO_UI_TOPMOST_NEVER) {
        const BOOL restored =
            ::SetWindowPos(host_hwnd, nullptr, desired.x, desired.y, desired.width, desired.height,
    ...
    if (handle->policy == SAO_UI_TOPMOST_ALWAYS ||
        (game_present && game != nullptr && game_is_topmost)) {
        succeeded = ::SetWindowPos(host_hwnd, HWND_TOPMOST, desired.x, desired.y, desired.width,
                                   desired.height, SWP_NOACTIVATE);
        ++handle->status.branch_game_topmost_count;
    } else if (game_present && game != nullptr) {
        succeeded = ::SetWindowPos(host_hwnd, game, desired.x, desired.y, desired.width,
                                   desired.height, SWP_NOACTIVATE);
...
extern "C" sao_status_t SAO_UI_CALL
sao_ui_z_order_hide_exstyle_mask(sao_ui_z_order_manager_handle_t handle, uint32_t mask) {
    if (handle == nullptr)
        return SAO_STATUS_ERR_HANDLE_INVALID;
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_z_order_status(sao_ui_z_order_manager_handle_t handle,

...
static_assert(SAO_UI_NERVEGEAR_SIZE == 72,
              "NerveGear disc size drifted from Python source");
static_assert(SAO_UI_NG_STATE_IDLE == 0,
              "state enum head drifted");
static_assert(SAO_UI_NG_STATE_LOGOUT == 6,
              "state enum tail drifted");
...
bool is_legal_forced_transition(SaoUiNerveGearState from,
                                SaoUiNerveGearState to) {
    if (to == SAO_UI_NG_STATE_IDLE) return true;   // abort is always ok
    if (from == SAO_UI_NG_STATE_IDLE   && to == SAO_UI_NG_STATE_LINKING) return true;
    if (from == SAO_UI_NG_STATE_LINKED && to == SAO_UI_NG_STATE_LOGOUT)  return true;
    return false;
}
...
float compute_linking_progress(int32_t elapsed_ms,
                               const SaoUiLinkStartTimeline& tl) {
    if (tl.total_duration <= 0.0f) return 1.0f;
...
extern "C" const SaoUiLinkStartTimeline* SAO_UI_CALL
sao_ui_nervegear_default_timeline(void) {
    return &kDefaultTimeline;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_create(
    ...
    *out_handle = nullptr;
    if (palette != SAO_UI_NG_PALETTE_DARK &&
        palette != SAO_UI_NG_PALETTE_LIGHT) {
...
extern "C" void SAO_UI_CALL sao_ui_nervegear_destroy(
    sao_ui_nervegear_handle_t handle) {
    delete handle;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_show(
    sao_ui_nervegear_handle_t handle) {
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_hide(
    sao_ui_nervegear_handle_t handle) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    std::lock_guard<std::mutex> lock(handle->mtx);
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_raise_topmost(
    sao_ui_nervegear_handle_t handle) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_set_position(
    sao_ui_nervegear_handle_t handle,
    int32_t x, int32_t y) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    std::lock_guard<std::mutex> lock(handle->mtx);
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_get_position(
    sao_ui_nervegear_handle_t handle,
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_set_palette(
    sao_ui_nervegear_handle_t handle,
    SaoUiNerveGearPalette palette) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (palette != SAO_UI_NG_PALETTE_DARK &&
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_get_state(
    sao_ui_nervegear_handle_t handle,
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_transition(
    sao_ui_nervegear_handle_t handle,
    SaoUiNerveGearState target_state) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (target_state < SAO_UI_NG_STATE_IDLE ||
        target_state > SAO_UI_NG_STATE_LOGOUT) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    ...
    if (should_fire) fire_event(handle, evt_to_fire, fire_x, fire_y);
    return SAO_STATUS_OK;
}
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_set_alpha(
    sao_ui_nervegear_handle_t handle, float alpha) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (!(alpha == alpha)) return SAO_STATUS_ERR_INVALID_ARGUMENT;  // NaN guard
...
extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_set_glow_phase(
    sao_ui_nervegear_handle_t handle, float glow_phase) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (!(glow_phase == glow_phase)) return SAO_STATUS_ERR_INVALID_ARGUMENT;
    std::lock_guard<std::mutex> lock(handle->mtx);
    handle->glow_phase = glow_phase;
    return SAO_STATUS_OK;
}

extern "C" sao_status_t SAO_UI_CALL sao_ui_nervegear_get_hit_shape(
    sao_ui_nervegear_handle_t handle,
    int32_t* out_center_x,
    int32_t* out_center_y,
    int32_t* out_radius) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    if (out_center_x == nullptr || out_center_y == nullptr ||
        out_radius == nullptr) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    std::lock_guard<std::mutex> lock(handle->mtx);
...
sao_ui_nervegear_tick(sao_ui_nervegear_handle_t handle, int32_t dt_ms) {
    ...
    int32_t             fire_x = 0, fire_y = 0;
    {
        ...
        handle->glow_phase = static_cast<float>(
            std::fmod(static_cast<double>(handle->glow_phase) +
                      static_cast<double>(dt_ms) * kGlowRateRadPerMs,
                      2.0 * 3.14159265358979323846));
        handle->state_elapsed_ms += dt_ms;
        switch (handle->state) {
            case SAO_UI_NG_STATE_LINKING: {
                const float p = compute_linking_progress(
                    handle->state_elapsed_ms, handle->timeline);
                if (p >= 1.0f) {
...
extern "C" SAO_UI_API sao_status_t SAO_UI_CALL
sao_ui_nervegear_get_link_progress(sao_ui_nervegear_handle_t handle,
                                   float* out_progress) {
    if (handle == nullptr) return SAO_STATUS_ERR_HANDLE_INVALID;
    ...
    switch (handle->state) {
        case SAO_UI_NG_STATE_LINKING:
            *out_progress = compute_linking_progress(
                handle->state_elapsed_ms, handle->timeline);
            break;
...
extern "C" SAO_UI_API sao_status_t SAO_UI_CALL
sao_ui_nervegear_hit_test(sao_ui_nervegear_handle_t handle,
...
sao_ui_nervegear_on_mouse_enter(sao_ui_nervegear_handle_t handle) {
    ...
    if (should_fire) fire_event(handle, SAO_UI_NG_EV_HOVER_ENTER, fx, fy);
    return SAO_STATUS_OK;
}
...
sao_ui_nervegear_on_mouse_leave(sao_ui_nervegear_handle_t handle) {
    ...
    if (should_fire) fire_event(handle, SAO_UI_NG_EV_HOVER_LEAVE, fx, fy);
    return SAO_STATUS_OK;
}
...
sao_ui_nervegear_on_mouse_up(sao_ui_nervegear_handle_t handle) {
    ...
    if (should_fire_click) fire_event(handle, SAO_UI_NG_EV_LEFT_CLICK,   fx, fy);
    if (should_fire_link)  fire_event(handle, SAO_UI_NG_EV_LINK_STARTED, fx, fy);
    return SAO_STATUS_OK;
}

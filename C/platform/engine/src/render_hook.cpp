...
struct sao_engine_render_hook_registry_s {
    ...
    std::unordered_map<std::string, std::vector<std::shared_ptr<HookEntry>>>
        hooks;
    std::unordered_map<sao_engine_hook_token_t, std::shared_ptr<HookEntry>>
        hooks_by_token;
    std::unordered_map<OverlayKey, std::vector<uint8_t>, OverlayKeyHash>
        overlays;
    std::unordered_map<std::string,
                       std::vector<std::shared_ptr<ClockHookEntry>>>
        clock_hooks;
    std::unordered_map<sao_engine_hook_token_t,
                       std::shared_ptr<ClockHookEntry>>
...
bool valid_dispatch_flags(uint32_t flags) noexcept {
    ...
    return (flags & ~kSources) == 0 && source != 0 &&
           (source & (source - 1u)) == 0;
}

std::vector<std::shared_ptr<ClockHookEntry>> snapshot_clock_hooks(
    ...
    std::stable_sort(result.begin(), result.end(),
                     [](const auto& lhs, const auto& rhs) {
                         ...
                         return lhs->insertion_sequence <
                                rhs->insertion_sequence;
                     });
    return result;
}

std::vector<std::shared_ptr<HookEntry>> snapshot_hooks(
    sao_engine_render_hook_registry_handle_t handle,
    const std::string& surface_id) {
...
sao_status_t invoke_hook(const HookEntry& hook,
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
        ...
        try {
            status = hook.callback(surface_id, input.data(), input.size(),
                                   candidate.data(), candidate.size(), &written,
                                   hook.user_data);
        } catch (...) {
...
extern "C" sao_status_t SAO_ENGINE_CALL sao_engine_render_hook_registry_create(
...
extern "C" void SAO_ENGINE_CALL sao_engine_render_hook_registry_destroy(
    sao_engine_render_hook_registry_handle_t handle) {
    delete handle;
}
...
sao_engine_render_hook_provider_status(
    sao_engine_render_hook_registry_handle_t handle) {
    return handle != nullptr ? SAO_STATUS_ERR_NOT_IMPLEMENTED
...
extern "C" sao_status_t SAO_ENGINE_CALL sao_engine_render_hook_register(
    ...
    if (handle == nullptr || !valid_string(plugin_id) ||
        !valid_string(surface_id) || !std::isfinite(priority) ||
    ...
    try {
        ...
        hook->insertion_sequence = handle->next_insertion_sequence.fetch_add(
...
extern "C" sao_status_t SAO_ENGINE_CALL sao_engine_render_clock_register(
    ...
    if (handle == nullptr || !valid_string(plugin_id) ||
        !valid_string(surface_id) || !valid_clock_point(hook_point) ||
        !std::isfinite(priority) || callback == nullptr ||
        out_token == nullptr) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    try {
        ...
        hook->user_data = user_data;
        {
            ...
            auto [bucket, bucket_inserted] =
                handle->clock_hooks.try_emplace(hook->surface_id);
            try {
...
extern "C" sao_status_t SAO_ENGINE_CALL sao_engine_render_clock_unregister(
    ...
    try {
        ...
        if (bucket != handle->clock_hooks.end()) {
            auto& hooks = bucket->second;
            std::erase_if(hooks, [token](const auto& candidate) {
                return candidate->token == token;
            });
            if (hooks.empty()) {
                handle->clock_hooks.erase(bucket);
...
extern "C" sao_status_t SAO_ENGINE_CALL sao_engine_render_clock_dispatch(
    ...
    if (handle == nullptr || !valid_string(surface_id) ||
        !valid_clock_point(hook_point) ||
        !valid_dispatch_flags(dispatch_flags) || viewport_width_px < 0 ||
    ...
    try {
        {
            std::lock_guard lock(handle->clock_mutex);
            if (handle->clock_initialized &&
                monotonic_time_ns < handle->last_clock_time_ns) {
                return SAO_STATUS_ERR_INVALID_ARGUMENT;
            }
            if (!handle->clock_initialized ||
                monotonic_time_ns > handle->last_clock_time_ns) {
                const uint64_t delta_ns = handle->clock_initialized
                                              ? monotonic_time_ns -
                                                    handle->last_clock_time_ns
                                              : 0;
                ...
                handle->clock_frame_delta_us = static_cast<uint32_t>(
                    std::min<uint64_t>(delta_ns / 1000u,
                                       std::numeric_limits<uint32_t>::max()));
            }

            auto& surface_clock = handle->surface_clocks[surface_id];
            const uint64_t surface_generation =
                handle->redraw_generations[surface_id];
            const bool redraw_changed =
                surface_clock.seen_global_redraw_generation !=
                    handle->global_redraw_generation ||
                surface_clock.seen_surface_redraw_generation !=
                    surface_generation;
            ...
            if (surface_clock.redraw_requested) {
                payload.flags |=
                    SAO_ENGINE_RENDER_DISPATCH_REDRAW_REQUESTED;
            }
        }

        const auto hooks = snapshot_clock_hooks(handle, surface_id,
                                                hook_point);
        ...
        for (const auto& hook : hooks) {
            ...
            sao_status_t status = SAO_STATUS_ERR_UNKNOWN;
            try {
                status = hook->callback(hook_point, &payload,
                                        hook->user_data);
            } catch (...) {
...
extern "C" sao_status_t SAO_ENGINE_CALL sao_engine_render_hook_set_overlay(
    ...
    size_t spec_len) {
    if (handle == nullptr || !valid_string(plugin_id) ||
        !valid_string(surface_id) || (spec_len != 0 && spec == nullptr)) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    try {
        size_t normalized_size = 0;
        sao_status_t status = sao_engine_ui_spec_normalize(
            spec, spec_len, nullptr, 0, &normalized_size);
        if (status != SAO_STATUS_OK) {
            return status;
        }
        std::vector<uint8_t> owned_spec(normalized_size);
        status = sao_engine_ui_spec_normalize(
            spec, spec_len, owned_spec.data(), owned_spec.size(),
            &normalized_size);
        if (status != SAO_STATUS_OK) {
            return status;
        }
        std::unique_lock lock(handle->mutex);
        handle->overlays[OverlayKey{plugin_id, surface_id}] =
            std::move(owned_spec);
        return SAO_STATUS_OK;
    } catch (...) {
        return SAO_STATUS_ERR_UNKNOWN;
...
extern "C" sao_status_t SAO_ENGINE_CALL sao_engine_render_hook_clear_overlay(
    sao_engine_render_hook_registry_handle_t handle,
    const char* plugin_id,
    const char* surface_id) {
    if (handle == nullptr || !valid_string(plugin_id)) {
    ...
    try {
        ...
        size_t removed = 0;
        for (auto entry = handle->overlays.begin();
             entry != handle->overlays.end();) {
...
extern "C" sao_status_t SAO_ENGINE_CALL sao_engine_render_hook_dispatch(
    ...
    if (handle == nullptr || !valid_string(surface_id) ||
        (input_len != 0 && input == nullptr) || out_written == nullptr) {
        return SAO_STATUS_ERR_INVALID_ARGUMENT;
    }
    try {
        ...
        for (const auto& hook : hooks) {
            ...
            const sao_status_t status =
                invoke_hook(*hook, surface_id, current, &replacement);
            if (status == SAO_STATUS_OK && !replacement.empty()) {
                current = std::move(replacement);
            }
        }
        *out_written = current.size();
        if (current.size() > out_capacity ||
            (!current.empty() && out_json == nullptr)) {
            return SAO_STATUS_ERR_BUFFER_TOO_SMALL;
        }
        if (!current.empty()) {

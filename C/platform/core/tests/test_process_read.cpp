...
TEST_CASE("process_open_current_pid_succeeds", "[core][process][runtime]") {
    const uint32_t pid = self_pid();
    REQUIRE(pid > 0u);

    sao_core_process_handle_t p = nullptr;
    const sao_status_t rc =
        sao_core_process_open(pid, SAO_PROCESS_ACCESS_INFO | SAO_PROCESS_ACCESS_READ, &p);
...
TEST_CASE("process_read_own_memory_roundtrip", "[core][process][memory][runtime]") {
    const uint32_t pid = self_pid();
    sao_core_process_handle_t p = nullptr;
    ...
    size_t bytes_read = 0;
    const sao_status_t rc = sao_core_mem_read(p, reinterpret_cast<uint64_t>(src.data()), dst.data(),
                                              sig_len, &bytes_read);
    REQUIRE(rc == SAO_STATUS_OK);
...
TEST_CASE("process_enum_modules_includes_kernel32", "[core][process][modules][runtime]") {
#if !defined(_WIN32)
    ...
    size_t names_used = 0;
    REQUIRE(sao_core_process_enum_modules(p, nullptr, 0, &entry_count, nullptr, 0, &names_used) ==
            SAO_STATUS_OK);
    ...
    REQUIRE(sao_core_process_enum_modules(p, entries.data(), entries.size(), &entry_count,
...
TEST_CASE("process_enum_regions_returns_committed", "[core][process][memory][runtime]") {
#if !defined(_WIN32)
    SUCCEED("Windows-only test");
    return;
#else
    ...
    REQUIRE(sao_core_mem_enum_regions(p, regions.data(), regions.size(), &region_count) ==
            SAO_STATUS_OK);
    REQUIRE(region_count > 0);
    ...
    for (const auto& r : regions) {
        if (probe_addr >= r.base_address && probe_addr < r.base_address + r.region_size) {
            probe_in_range = true;
            break;
        }
    }
...
TEST_CASE("process_read_multiple_requests_return_independent_status",
    ...
    const sao_status_t status_a = sao_core_mem_read(p, reinterpret_cast<uint64_t>(text_a), dst_a,
                                                    sizeof(text_a), &bytes_read_a);

    size_t bytes_read_bogus = 0;
    const sao_status_t status_bogus = sao_core_mem_read(p, 0xFFFFFFFFFFFF0000ULL, dst_bogus,
                                                        sizeof(dst_bogus), &bytes_read_bogus);

    size_t bytes_read_b = 0;
    const sao_status_t status_b = sao_core_mem_read(p, reinterpret_cast<uint64_t>(text_b), dst_b,
                                                    sizeof(text_b), &bytes_read_b);
...
TEST_CASE("process_read_pointer_chain_zero_offset_is_base",
    ...
    uint64_t out = 0;
    REQUIRE(sao_core_mem_read_pointer_chain(p, 0xDEADBEEFULL, nullptr, 0, &out) == SAO_STATUS_OK);
    CHECK(out == 0xDEADBEEFULL);

    // One-hop: pointer stored inside a local variable, offset 0.
    uint64_t hidden = 0x1234567890ABCDEFULL;
    int32_t offsets[1] = {0};
    REQUIRE(sao_core_mem_read_pointer_chain(p, reinterpret_cast<uint64_t>(&hidden), offsets, 1,
                                            &out) == SAO_STATUS_OK);

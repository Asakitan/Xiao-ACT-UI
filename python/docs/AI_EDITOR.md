...
- **右侧边栏**：对话面板。顶部是 Provider 标签页（Assistant、Claude Code、Codex 等），按已配置的 API Key 动态出现。宽度可拖拽（260-600px）。
...
状态栏显示当前模式，点击聊天输入框左侧的模式按钮可选择：

| 模式 | 说明 | LLM 可用工具范围 |
|------|------|------------------|
| **Agent** | 全自主 | 所有工具自动可用，无需确认 |
| **Ask** | 只读问答 | 可以读文件和查询，不能写文件或执行命令 |
| **Plan** | 规划模式 | 可以读文件，写文件和终端操作需要确认。LLM 输出实现方案而非直接执行，底部出现「Implement」按钮，点击自动切换到 Agent 模式执行方案 |

每个工具还可以单独设置权限覆盖（allowed / confirm / disabled）。

### 审批级别

在模式选择弹出菜单中，还可以设置审批级别：

| 级别 | 图标 | 说明 |
|------|------|------|
| **Default Approvals** | ◎ | 按模式的默认权限表执行，写操作需要逐一确认 |
| **Bypass Approvals** | ⚡ | 自动批准所有工具调用，跳过确认弹窗 |
| **Autopilot** | ✦ | 完全自主循环，自动批准且不等待用户交互 |

审批级别与模式独立——例如在 Plan 模式 + Bypass 级别下，写操作的确认会被自动跳过。

# -*- coding: utf-8 -*-
"""RW module — local player state reader.

Reads the local player pawn from dwLocalPlayerPawn and extracts all
fields needed by tracker, RCS, reactor, and visibility check:
...
def read_local_player(read_fn, module_base: int,
    ...
    # Map name: dwGlobalVars → +0x1B8 → string ptr → name
    dw_gv = g_off.get("dwGlobalVars", 0)
    if dw_gv and module_base:
        gv_raw = read_fn(module_base + dw_gv, 8)
        if gv_raw and len(gv_raw) >= 8:
            gv_ptr = struct.unpack_from("<Q", gv_raw)[0]
            if gv_ptr:
                mn_ptr_raw = read_fn(gv_ptr + 0x1B8, 8)
                if mn_ptr_raw and len(mn_ptr_raw) >= 8:
                    mn_ptr = struct.unpack_from("<Q", mn_ptr_raw)[0]
                    if mn_ptr:
                        mn_raw = read_fn(mn_ptr, 64)
                        if mn_raw:
                            try:
                                result["map_name"] = mn_raw.split(b"\x00", 1)[0].decode("utf-8", "replace")
                            except Exception:
                                pass

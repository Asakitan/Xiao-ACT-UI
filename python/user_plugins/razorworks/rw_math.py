# -*- coding: utf-8 -*-
"""RW module — vector/angle math for tracker, RCS, projection.

Pure functions, no side effects, no imports beyond stdlib math.
All angles are in degrees; Source 2 convention: pitch down = positive,
...
def angle_to(src: Vec3, dst: Vec3) -> Tuple[float, float]:
    ...
    Returns angles in degrees (Source 2 convention: pitch positive = down).
    """
    dx = dst[0] - src[0]
    dy = dst[1] - src[1]
    dz = dst[2] - src[2]
...
def angle_to_pixels(pitch_delta: float, yaw_delta: float,
    ...
    Source 2 formula: 1 pixel ≈ sensitivity * 0.022 degrees (at default FOV).

# -*- coding: utf-8 -*-
"""target Overlay plugin — client.dll pattern scanner.
...
target Steam updates without manually refreshing ``offsets.json``: the
...
def scan_module(read_fn, module_base: int, module_size: int,
    ...
    callback (typically ``DataSrc.read``). Returns ``{name: rva}``;
...
# ── target pattern definitions (from a2x/cs2-dumper src/analysis/offsets.rs) ──
# These are the opcode sequences immediately around the RIP-relative
# reference that stores/loads each global. They survive most target updates
...
PATTERNS = {
    ...
    # dwGlobalVars
    "dwGlobalVars": "48 89 15 {rip} 48 89 42",
    # dwPlantedC4
    "dwPlantedC4": "48 8B 15 {rip} FF C0 48 8D 4C 24",
    # dwGameRules
    "dwGameRules": "48 8B 0D {rip} 48 85 C9 74 ?? 45",
}

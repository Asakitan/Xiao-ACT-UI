# -*- coding: utf-8 -*-
"""RW module — Engine A reader facade.

Extended to support full analysis features: local player state, bone
...
from .rw_mesh import read_bones, head_position
from .rw_ctx import read_local_player
from .rw_resolver import resolve_offsets
from .rw_schema import SchemaConfig
from .rw_scan import Pattern, default_patterns, scan_module
...
class DataSrc:
    ...
    def __init__(self, config: SchemaConfig, *, use_sig: bool = True) -> None:
        self._config = config
        self._attached_pid: int = 0
        self._module_base: int = 0
        self._module_size: int = 0
        self._last_error: str = ""
        self._use_sig = bool(use_sig)
    ...
    def attach(self, pid: int) -> bool:
        ...
        if self._use_sig:
            ...
            try:
                ...
                pass
        self._try_schema_resolve()
        return True

    def _find_module_base_by_name(self, name: str) -> int:
        """Find a module base in the target process by name (PEB LDR walk)."""
    ...
    def _try_schema_resolve(self) -> None:
        """Attempt runtime SchemaSystem resolution; merge into config on success."""
        try:
            resolved = resolve_offsets(self.read, self._find_module_base_by_name)
        except Exception:
            return
        if not resolved:
            return
        ef = resolved.get("entity_fields", {})
        sf = resolved.get("scene_node_fields", {})
        merged_e = 0
        for k, v in ef.items():
            if v and v > 0:
                self._config.entity_fields[k] = v
                merged_e += 1
        merged_s = 0
        for k, v in sf.items():
            if v and v > 0:
                self._config.scene_node_fields[k] = v
                merged_s += 1
        if merged_e or merged_s:
            self._config.config_version = f"schema-live-{merged_e}e{merged_s}s"
    ...
    def snapshot(self) -> dict:
        ...
        if self._use_sig and not self._scan_attempted:
        ...
        # Filter alive entities from Batch 3 results
        local_pawn = result["local_player"].get("pawn", 0) if result["local_player"] else 0
        local_index = -1
        alive = []  # (index, pawn, health, team, scene_node)
        for (i, pawn, base_off), block in zip(active, blocks):
            ...
            if pawn == local_pawn:
                local_index = i
                continue
            alive.append((i, pawn, health, team, scene_node))
        ...
        # Batch 4: origin + spotted + bone_array_ptr — all in one batch
        batch4_reqs = []
        for _, pawn, _, _, sn in alive:
            batch4_reqs.append((sn + origin_off, 12))                         # origin
            if spotted_state_off:
                batch4_reqs.append((pawn + spotted_state_off, 16))            # spotted
            else:
                batch4_reqs.append((0, 0))                                    # placeholder
            batch4_reqs.append((sn + model_state_off + bone_array_off, 8))    # bone array ptr
        batch4_data = self.read_batch(batch4_reqs) if batch4_reqs else []

        # Batch 5: bone data — batch all bone array reads
        bone_data_size = bone_count * 32
        batch5_reqs = []
        batch5_map = []  # index into alive
        for ai in range(len(alive)):
            ba_raw = batch4_data[ai * 3 + 2] if ai * 3 + 2 < len(batch4_data) else None
            ba_ptr = 0
            if ba_raw and len(ba_raw) >= 8:
                ba_ptr = struct.unpack_from("<Q", ba_raw)[0]
            if ba_ptr:
                batch5_reqs.append((ba_ptr, bone_data_size))
                batch5_map.append(ai)
            else:
                batch5_map.append(-1)
        batch5_data = self.read_batch(batch5_reqs) if batch5_reqs else []

        # Build entity dicts from batched results
        entities = []
        b5_idx = 0
        from .rw_mesh import parse_bone_positions, head_position as _head_pos
        for ai, (i, pawn, health, team, sn) in enumerate(alive):
            b4_base = ai * 3
            # Origin
            ori_raw = batch4_data[b4_base] if b4_base < len(batch4_data) else None
            if not ori_raw or len(ori_raw) < 12:
                continue
            try:
                ox, oy, oz = struct.unpack("<3f", ori_raw)
            except Exception:
                continue

            ent = {
                "index": i, "pawn": pawn, "health": health, "team": team,
                "origin": (ox, oy, oz),
            }

            # Spotted (from batch4)
            if spotted_state_off:
                sp_raw = batch4_data[b4_base + 1] if b4_base + 1 < len(batch4_data) else None
            ...
            # Bones (from batch5)
            if batch5_map[ai] >= 0:
                bd = batch5_data[b5_idx] if b5_idx < len(batch5_data) else None
                b5_idx += 1
                if bd:
                    bones = parse_bone_positions(bd, bone_count)
                    if bones:
                        ent["bones"] = bones
                        head = _head_pos(bones)
                        if head:
                            ent["head_pos"] = head

            entities.append(ent)

        # Visibility annotation (in-place, no copy)
        if local_index >= 0:
            from .rw_sight import is_visible_by_mask
            for ent in entities:
                mask = ent.get("spotted_mask", 0)
                ent["is_visible"] = is_visible_by_mask(mask, local_index) if mask else True
